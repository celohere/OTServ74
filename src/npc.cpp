...
void Npc::onUpdateTileItem(const Tile *tile,
                           const Position &pos,
                           uint32_t stackpos,
                           const Item *oldItem,
...
void Npc::onRemoveTileItem(const Tile *tile, const Position &pos, uint32_t stackpos, const ItemType &iType, const Item *item)
{
...
void Npc::onCreatureMove(const Creature *creature,
                         const Tile *newTile,
...
{
	if (creature->getID() == this->getID()) return;
...
void Npc::doSay(std::string msg, uint32_t delay)
{
	...
	createSchedulerTask(delay, boost::bind(&Game::internalCreatureSay, &g_game, this, SPEAK_SAY, msg)));
}
...
int NpcScriptInterface::luaGetDistanceTo(lua_State *L)
{
	...
	if (thing && npc) {
		...
		if (npc_pos.z != thing_pos.z) {
			...
			int32_t dist =
			std::max(std::abs(npc_pos.x - thing_pos.x), std::abs(npc_pos.y - thing_pos.y));
...
NpcScript::NpcScript(std::string file, Npc *npc) : NpcEventsHandler(npc)
{
	...
	if (m_scriptInterface->reserveScriptEnv()) {
		...
		if (m_scriptInterface->loadFile(file, false) == -1) {
			std::cout << "Warning: [NpcScript::NpcScript] Can not load script. " << file
			          << std::endl;
