...
class DatabaseDriver
{
	...
	*
	* Returns instance of database handler. Don't create database (or drivers)
	* instances in your code - instead of it use Database::instance(). This method
	* stores static instance of connection class internaly to make sure exacly one
	* instance of connection is created for entire system.
	*
	...
	*
	* Methods for starting, committing and rolling back transaction. Each of the
	* returns boolean value.
	*
	* @return true on success, false on error
	* @note
	*  If your database system doesn't support transactions you should return true
	* - it's not feature test, code should work without transaction, just will
	* lack integrity.
	*/
	...
	*/
	 ...
	 *
	 * @return id of last inserted row, 0 if last query did not result in any rows
	 * with auto_increment keys
	 */
...
class DBResult : public boost::enable_shared_from_this<DBResult>
{
	...
	*\return a PropStream that is initiated with the blob data field, if not exist
	*it returns NULL.
	*\param s The name of the field
	*/
 ...
 * Thread locking hack.
 *
 * By using this class for your queries you lock and unlock database for
 * threads.
*/
 ...
 * INSERT statement.
 *
 * Gives possibility to optimize multiple INSERTs on databases that support
 * multiline INSERTs.
 */
class DBInsert
{
	...
	*
	* On databases that doesn't support multiline INSERTs it simply execute INSERT
	* for each row.
	*
