...
GlobalEvents::GlobalEvents() : m_scriptInterface("GlobalEvent Interface")
{
	...
	if (globalEvent->getEventType() == GLOBALEVENT_TIMER) {
		...
		if (it == timerMap.end()) {
			...
			if (timerEventId == 0) {
				...
				createSchedulerTask(TIMER_INTERVAL, boost::bind(&GlobalEvents::timer, this)));
			}
			return true;
		}
	...
	} else // think event
	{
		...
		if (it == thinkMap.end()) {
			...
			if (thinkEventId == 0) {
				...
				createSchedulerTask(SCHEDULER_MINTICKS, boost::bind(&GlobalEvents::think, this)));
			}
			return true;
		}
	}
...
bool GlobalEvent::configureEvent(xmlNodePtr p)
{
	...
	if (readXMLString(p, "type", strValue)) {
		...
		if (atoiparams0 < 0 || atoiparams0 > 23) {
			std::cout << "[Error - GlobalEvent::configureEvent] No valid hour \"" << strValue
			          << "\" for globalevent with name " << m_name << std::endl;
			return false;
		}
		...
		if (params.size() > 1) {
			...
			if (atoiparams1 < 0 || atoiparams1 > 59) {
				std::cout
				<< "[Error - GlobalEvent::configureEvent] No valid minute \""
				<< strValue << "\" for globalevent with name " << m_name << std::endl;
				return false;
			}
			...
			if (params.size() > 2) {
				...
				if (atoiparams2 < 0 || atoiparams2 > 59) {
					std::cout << "[Error - GlobalEvent::configureEvent] No "
					             "valid second \""
					          << strValue << "\" for globalevent with name "
					          << m_name << std::endl;
					return false;
				}
