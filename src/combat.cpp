...
CombatSource::CombatSource() : creature(NULL), item(NULL), condition(false)
{
}
...
CombatSource::CombatSource(Creature *creature) : creature(creature), item(NULL), condition(false)
{
	if (creature) creature->addRef();
}

CombatSource::CombatSource(Item *item) : creature(NULL), item(item), condition(false)
{
	if (item) item->addRef();
}

CombatSource::CombatSource(const CombatSource &rhs) : creature(NULL), item(NULL)
{
...
/*
bool Combat::internalCombat(CombatSource& combatSource, CombatParams& params,
Creature* target,
  const SpectatorVec* spectators = NULL) const
{
  Creature* attacker = combatSource.getSourceCreature();
  if(!params.aggressive || (attacker != target && Combat::canDoCombat(attacker,
target) == RET_NOERROR)){
    if(params.combatType != COMBAT_NONE){
      ...
      if(g_game.combatBlockHit(params.combatType, combatSource, target, value,
...
bool isUnjustKill(const Creature *attacker, const Creature *target)
{
	...
	if (attackerPlayer == NULL || targetPlayer == NULL || attackerPlayer->isPartner(targetPlayer) ||
	    Combat::isInPvpZone(attackerPlayer, targetPlayer) || targetPlayer->hasAttacked(attackerPlayer) ||
	    targetPlayer->getSkull() != SKULL_NONE || targetPlayer == attackerPlayer) {
		return false;
	}
