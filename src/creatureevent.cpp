...
CreatureEvents::CreatureEvents() : m_scriptInterface("CreatureScript Interface")
{
...
bool CreatureEvents::registerEvent(Event *event, xmlNodePtr p)
{
	...
	case CREATURE_EVENT_NONE:
		std::cout
		<< "Error: [CreatureEvents::registerEvent] Trying to register event without type!."
		<< std::endl;
		return false;
		break;
...
bool CreatureEvent::configureEvent(xmlNodePtr p)
{
	...
	if (readXMLString(p, "type", str)) {
		if (asLowerCaseString(str) == "login") {
			...
			std::cout << "Error: [CreatureEvent::configureEvent] No valid type for "
			             "creature event."
			          << str << std::endl;
			return false;
		}
...
void CreatureEvent::executeOnKill(Creature *creature, Creature *target, bool lastHit)
{
	...
	if (m_scriptInterface->reserveScriptEnv()) {
		...
		std::cout << "[Error] __ENABLE_SERVER_DIAGNOSTIC__Call stack overflow. "
		             "CreatureEvent::executeOnKill"
		          << std::endl;
	}
}
