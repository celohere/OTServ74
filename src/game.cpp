...
void Game::start(ServiceManager *servicer)
{
	...
	checkLightEvent =
	g_scheduler.addEvent(createSchedulerTask(EVENT_LIGHTINTERVAL, boost::bind(&Game::checkLight, this)));
	checkCreatureLastIndex = 0;

	checkCreatureEvent = g_scheduler.addEvent(
	createSchedulerTask(EVENT_CREATURE_THINK_INTERVAL, boost::bind(&Game::checkCreatures, this)));

	checkDecayEvent =
	g_scheduler.addEvent(createSchedulerTask(EVENT_DECAYINTERVAL, boost::bind(&Game::checkDecay, this)));

	waitingScriptEvent = g_scheduler.addEvent(
	createSchedulerTask(EVENT_SCRIPT_CLEANUP_INTERVAL, boost::bind(&Game::scriptCleanup, this)));
}
...
bool Game::saveServer(ServerSaveType saveType)
{
	...
	if (!saveGameState()) std::cout << "Could not save global game state." << std::endl;
	...
	if (saveType == SERVER_SAVE_SHALLOW) return true;
	...
	std::cout << "Notice: Server saved. Process took " << (OTSYS_TIME() - start) / (1000.)
	          << "s." << std::endl;
...
void Game::loadGameState()
{
	...
	for (DBResult_ptr result = db->storeQuery("SELECT `id`, `value` FROM `global_storage`");
	     result; result = result->advance()) {
...
bool Game::saveGameState()
{
	...
	if (!global_stmt.execute()) return false;
...
void Game::runWaitingScripts()
{
	...
	waitingScriptEvent = g_scheduler.addEvent(
	createSchedulerTask(EVENT_SCRIPT_TIMER_INTERVAL, boost::bind(&Game::runWaitingScripts, this)));
}
...
bool Game::loadScripts()
{
	...
	if (script_environment || script_system) {
		...
		for (AutoList<Creature>::listiterator it = Game::listCreature.list.begin();
		     it != Game::listCreature.list.end(); ++it) {
	...
	try {
		...
		script_system->loadFile(g_config.getString(ConfigManager::DATA_DIRECTORY) +
		                        "scripts/main.lua");

		waitingScriptEvent =
		g_scheduler.addEvent(createSchedulerTask(EVENT_SCRIPT_TIMER_INTERVAL,
...
bool Game::getCustomValue(const std::string &key, std::string &value) const
{
	...
	if (i == globalStorage.end()) return false;
	value = i->second;
	return true;
}
...
void Game::refreshMap(Map::TileMap::iterator *map_iter, int clean_max)
{
	...
	if (!map_iter) map_iter = &begin_here;
	...
	for (; *map_iter != end_here && (clean_max == 0 ? true : (cleaned < clean_max)); ++*map_iter, ++cleaned) {
		...
		for (int32_t i = downItemSize - 1; i >= 0; --i) {
			...
			if (item) {
				...
				if (ret != RET_NOERROR) {
					std::cout << "Could not refresh item: " << item->getID()
					          << "pos: " << tile->getPosition() << std::endl;
				}
			}
		}
		...
		for(TileItemReverseIterator it = list.rbegin(); it != list.rend(); ++it){
		  ...
		  if(ret == RET_NOERROR){
		    ...
		    std::cout << "Could not refresh item: " << item->getID() << "pos: " <<
		tile->getPosition() << std::endl;
		    delete item;
		  }
		}
		*/
...
{
	if (!begin) begin = new Map::TileMap::iterator(map->refreshTileMap.begin());
	...
	g_scheduler.addEvent(createSchedulerTask(100, boost::bind(&Game::proceduralRefresh, this, begin)));
}
...
ReturnValue Game::internalUseItemEx(Player *player,
	...
	if (pos.x != 0xFFFF) {
		...
		if (tile) {
			...
			/*use item*/
				...
				// First check top downItem and if it matches the spriteID client
				// requested
...
void Game::internalGetPosition(Item *item, Position &pos, uint8_t &stackpos)
{
	...
	if (topParent) {
		if (Player *player = dynamic_cast<Player *>(topParent)) {
			...
			if (container) {
				pos.y = ((uint16_t)((uint16_t)0x40) |
				         ((uint16_t)player->getContainerID(container)));
...
Creature *Game::getCreatureByName(const std::string &s)
{
	...
	for (AutoList<Creature>::listiterator it = listCreature.list.begin();
	     it != listCreature.list.end(); ++it) {
		if (!it->second->isRemoved()) {
			std::string txt2 = asUpperCaseString(it->second->getName());
			if (txt1 == txt2) return it->second;
		}
	}
...
std::vector<Creature *> Game::getCreaturesByName(const std::string &s)
{
	...
	for (AutoList<Creature>::listiterator it = listCreature.list.begin();
		...
		if (!it->second->isRemoved()) {
			std::string txt2 = asUpperCaseString(it->second->getName());
			if (txt1 == txt2) creaturesFound.push_back(it->second);
		}
	}
...
std::vector<Player *> Game::getPlayersByName(const std::string &s)
{
	...
	for (AutoList<Player>::listiterator it = Player::listPlayer.list.begin();
		...
		if (!it->second->isRemoved()) {
			std::string txt2 = asUpperCaseString(it->second->getName());
			if (txt1 == txt2) playersFound.push_back(it->second);
		}
	}
...
#ifdef __DEBUG__
			std::cout << "Failure: [Game::getPlayerByNameEx], can not load player: " << s
			          << std::endl;
#endif
			delete player;
			return NULL;
...
#ifdef __DEBUG__
			std::cout << "Failure: [Game::getPlayerByGuidEx], can not load player: " << name
			          << std::endl;
#endif
			delete player;
			player = NULL;
...
{
	if (s.empty()) return std::vector<Player *>();

	if ((*s.rbegin()) != '~') return getPlayersByName(s);
...
bool Game::internalPlaceCreature(Creature *creature, const Position &pos, bool extendedPos /*=false*/, bool forced /*= false*/)
{
	...
	// std::cout << "internalPlaceCreature: " << creature << " " <<
	// creature->getID() << std::endl;
...
bool Game::playerMoveThing(uint32_t playerId,
                           ...
                           uint8_t count)
{
	...
	if (Creature *movingCreature = thing->getCreature()) {
		if (Position::areInRange<1, 1, 0>(movingCreature->getPosition(), player->getPosition())) {
			SchedulerTask *task =
			createSchedulerTask(2000, boost::bind(&Game::playerMoveCreature, this,
			                                      player->getID(), movingCreature->getID(),
			                                      movingCreature->getPosition(),
			                                      toCylinder->getPosition()));
			player->setNextActionTask(task);
		} else {
			playerMoveCreature(playerId, movingCreature->getID(),
			                   movingCreature->getPosition(), toCylinder->getPosition());
		}
...
bool Game::playerMoveCreature(uint32_t playerId,
                              ...
                              const Position &toPos)
{
	...
	if (!player->canDoAction()) {
		...
		SchedulerTask *task =
		createSchedulerTask(delay, boost::bind(&Game::playerMoveCreature, this, playerId,
		                                       movingCreatureId, movingCreatureOrigPos, toPos));
		player->setNextActionTask(task);
		return false;
	}
	...
	if (!movingCreature || movingCreature->isRemoved()) return false;
	...
	if (!Position::areInRange<1, 1, 0>(movingCreatureOrigPos, player->getPosition())) {
		...
		if (getPathToEx(player, movingCreatureOrigPos, listDir, 0, 1, true, true)) {
			...
			createTask(boost::bind(&Game::playerAutoWalk, this, player->getID(), listDir)));

			SchedulerTask *task =
			createSchedulerTask(1500, boost::bind(&Game::playerMoveCreature, this, playerId,
			                                      movingCreatureId, movingCreatureOrigPos, toPos));
	...
	if (player != movingCreature) {
		if (toTile->blockProjectile()) {
			...
			return false;
		           ...
		           !toTile->hasFlag(TILEPROP_PROTECTIONZONE)) {
			player->sendCancelMessage(RET_NOTPOSSIBLE);
			return false;
	...
	ReturnValue ret =
	internalMoveCreature(player, movingCreature, movingCreature->getParentTile(), toTile);
	if (ret != RET_NOERROR) {
...
bool Game::onAccountLogin(std::string &name,
                          uint32_t &number,
                          std::string &password,
                          time_t &premiumEnd,
                          uint32_t &warnings,
...
{
	if (!script_system) return false; // Not handled
...
{
	if (!script_system) return; // Not handled
...
ReturnValue Game::internalMoveCreature(Creature *actor, Creature *creature, Direction direction, uint32_t flags /*= 0*/)
{
	...
	if (creature->getPlayer() && canChangeFloor) {
		...
		if (currentPos.z != 8 && creature->getParentTile()->hasHeight(3)) {
			...
			if (currentPos.z != 7 &&
			    (tmpTile == NULL || (tmpTile->ground == NULL && !tmpTile->blockSolid()))) {
				tmpTile = getParentTile(destPos.x, destPos.y, destPos.z + 1);
...
ReturnValue
Game::internalMoveCreature(Creature *actor, Creature *creature, Cylinder *fromCylinder, Cylinder *toCylinder, uint32_t flags /*= 0*/)
{
...
bool Game::playerMoveItem(uint32_t playerId,
                          const Position &fromPos,
                          uint16_t spriteId,
                          uint8_t fromStackPos,
                          const Position &toPos,
                          uint8_t count)
{
	...
	if (!player->canDoAction()) {
		uint32_t delay = player->getNextActionTime();
		SchedulerTask *task =
		createSchedulerTask(delay, boost::bind(&Game::playerMoveItem, this, playerId, fromPos,
	...
	if (!Position::areInRange<1, 1, 0>(playerPos, mapFromPos)) {
		...
		if (getPathToEx(player, item->getPosition(), listDir, 0, 1, true, true)) {
			...
			SchedulerTask *task =
			createSchedulerTask(400, boost::bind(&Game::playerMoveItem, this, playerId, fromPos,
			                                     spriteId, fromStackPos, toPos, count));
			player->setNextWalkActionTask(task);
			return true;
		} else {
	...
	ReturnValue ret =
	internalMoveItem(player, fromCylinder, toCylinder, toIndex, item, count, NULL, 0, retVal);
	if (ret != RET_NOERROR) {
		player->sendCancelMessage(ret);
		return false;
	}
...
ReturnValue Game::internalMoveItem(Creature *actor,
                                   ...
                                   ReturnValue retVal /*= RET_NOERROR*/)
{
	...
	if (script_system) {
		if (fromCylinder) {
			if (fromCylinder->getCreature()) {
				...
				Script::OnEquipItem::Event evt(player, item,
				                               SlotType(fromCylinder->__getIndexOfThing(item)),
				                               false, ret);
				if (script_system->dispatchEvent(evt)) {
					// handled by script
					return ret;
	...
	// retVal contains the caller functions ReturnValue, it's checked here after
	// the
	...
	if (ret == RET_NEEDEXCHANGE) {
		...
		if (ret == RET_NOERROR) {
			...
			fromCylinder->__queryMaxCount(-1, toItem, toItem->getItemCount(),
			...
			if ((toCylinder->__queryRemove(toItem, toItem->getItemCount(), flags) == RET_NOERROR) &&
				...
				if (oldToItemIndex != -1) {
					toCylinder->postRemoveNotification(actor, toItem, fromCylinder,
					                                   oldToItemIndex, true);
				}
...
ReturnValue Game::internalAddItem(Creature *actor,
                                  Cylinder *toCylinder,
                                  Item *item,
...
ReturnValue
Game::internalAddItem(Creature *actor, Cylinder *toCylinder, Item *item, int32_t index, uint32_t flags, bool test, uint32_t &remainderCount)
{
	remainderCount = 0;
	...
	original cylinder,
	since the queryDestination can return a cylinder that might only hold a part
	of the full amount.
	*/
	uint32_t maxQueryCount = 0;
	...
	if (!test) {
		if (item->isStackable() && toItem && toItem->getID() == item->getID()) {
			...
			if (toItem->getID() == item->getID()) {
				n = std::min((uint32_t)100 - toItem->getItemCount(), m);
				toCylinder->__updateThing(actor, toItem, toItem->getID(),
			...
			if (m - n > 0) {
				if (m - n != item->getItemCount()) {
					...
					if (internalAddItem(actor, origToCylinder, remainderItem,
					                    INDEX_WHEREEVER, flags, false) != RET_NOERROR) {
...
ReturnValue
Game::internalRemoveItem(Creature *actor, Item *item, int32_t count /*= -1*/, bool test /*= false*/, uint32_t flags /*= 0*/)
{
	...
	if (script_system) {
		if (cylinder->getCreature()) {
			...
			Script::OnEquipItem::Event evt(player, item,
			                               SlotType(cylinder->__getIndexOfThing(item)),
			                               false, ret);
			if (script_system->dispatchEvent(evt)) {
				// handled by script
				return ret;
...
uint32_t Game::getMoney(const Cylinder *cylinder)
{
	...
	for (int i = cylinder->__getFirstIndex(); i < cylinder->__getLastIndex(); ++i) {
		if (!(thing = cylinder->__getThing(i))) continue;

		if (!(item = thing->getItem())) continue;
	...
	for (std::map<uint32_t, ItemType *>::reverse_iterator it = Item::items.currencyMap.rbegin();
		...
		if (money >= it->first) {
			...
			while (count > 0) {
				...
				ReturnValue ret =
				internalAddItem(actor, cylinder, moneyItem, INDEX_WHEREEVER, flags);
				if (ret != RET_NOERROR) {
					if (internalAddItem(actor, cylinder->getParentTile(), moneyItem,
					                    INDEX_WHEREEVER) != RET_NOERROR) {
						FreeThing(moneyItem);
					}
				}
...
Item *Game::transformItem(Creature *actor, Item *item, uint16_t newId, int32_t newCount /*= -1*/)
{
	...
	if (curType.alwaysOnTop != newType.alwaysOnTop) {
		...
		if (script_environment != NULL) script_environment->reassignThing(item, newItem);
...
ReturnValue Game::internalTeleport(Creature *actor, Thing *thing, const Position &newPos, uint32_t flags /*= 0*/)
{
	...
	if (!toTile) return RET_NOTPOSSIBLE;

	if (!thing->getParentTile()) {
		if (thing->getCreature()) {
			if (placeCreature(thing->getCreature(), newPos, false,
			                  ((flags & FLAG_IGNOREBLOCKITEM) || (flags & FLAG_IGNOREBLOCKCREATURE))))
				return RET_NOERROR;
		} else {
	...
	if (newPos == thing->getPosition()) return RET_NOERROR;
	if (thing->isRemoved()) return RET_NOTPOSSIBLE;

	if (Creature *creature = thing->getCreature()) {
		creature->getParentTile()->moveCreature(actor, creature, toTile, true);
		return RET_NOERROR;
	} else if (Item *item = thing->getItem()) {
		return internalMoveItem(actor, item->getParent(), toTile, INDEX_WHEREEVER, item,
...
{
	if (type < MSG_CLASS_FIRST || type > MSG_CLASS_LAST) return false;
...
bool Game::playerMove(uint32_t playerId, Direction dir)
{
	...
	if (delay > 0) {
		...
		SchedulerTask *task =
		createSchedulerTask(((uint32_t)delay), boost::bind(&Game::playerMove, this, playerId, dir));
...
{
	if (!player->hasFlag(PlayerFlag_CanBroadcast)) return false;
...
{
	if (!g_game.script_system) return;
...
bool Game::playerUseItem(uint32_t playerId, Position pos, uint8_t stackPos, uint8_t index, uint16_t spriteId, bool isHotkey)
{
	Player *player = getPlayerByID(playerId);
	if (!player || player->isRemoved()) return false;
	...
	if (!player->canDoAction()) {
		uint32_t delay = player->getNextActionTime();
		SchedulerTask *task =
		createSchedulerTask(delay, boost::bind(&Game::playerUseItem, this, playerId, pos,
		                                       stackPos, index, spriteId, isHotkey));
		player->setNextActionTask(task);
		return false;
	}
	...
	if (ret != RET_NOERROR) {
		if (ret == RET_ITEMOUTORANGE) {
			...
			if (getPathToEx(player, pos, listDir, 0, 1, true, true)) {
				...
				boost::bind(&Game::playerAutoWalk, this, player->getID(), listDir)));

				SchedulerTask *task =
				createSchedulerTask(400, boost::bind(&Game::playerUseItem, this, playerId,
				                                     pos, stackPos, index, spriteId, isHotkey));
				player->setNextWalkActionTask(task);
				return true;
			}
...
bool Game::playerUseItemEx(uint32_t playerId,
                           Position fromPos,
                           uint8_t fromStackPos,
                           ...
                           bool isHotkey)
{
	...
	if (!player->canDoAction()) {
		uint32_t delay = player->getNextActionTime();
		SchedulerTask *task =
		createSchedulerTask(delay, boost::bind(&Game::playerUseItemEx, this, playerId,
		                                       fromPos, fromStackPos, fromSpriteId, toPos,
		                                       toStackPos, toSpriteId, isHotkey));
		player->setNextActionTask(task);
		return false;
	}
	...
	ReturnValue ret = internalUseItemEx(player, PositionEx(fromPos, fromStackPos),
	                                    PositionEx(toPos, toStackPos), item, NULL, targetItem, isHotkey);
	...
	if (ret != RET_NOERROR) {
		if (ret == RET_NEEDTOPICKUPITEM) {
			...
			if (ret == RET_NOERROR) {
				// changing the position since its now in the inventory of the
				// player
				internalGetPosition(moveItem, fromPos, fromStackPos);
				ret = internalUseItemEx(player, PositionEx(fromPos, fromStackPos),
				                        PositionEx(toPos, toStackPos), item, NULL,
				                        NULL, isHotkey);
			}
		}
		...
		if (ret == RET_ITEMOUTORANGE) {
			...
			if (getPathToEx(player, fromPos, listDir, 0, 1, true, true)) {
				...
				SchedulerTask *task =
				createSchedulerTask(400, boost::bind(&Game::playerUseItemEx, this, playerId,
				                                     fromPos, fromStackPos, fromSpriteId, toPos,
				                                     toStackPos, toSpriteId, isHotkey));
				player->setNextWalkActionTask(task);
				return true;
...
bool Game::playerUseBattleWindow(uint32_t playerId,
                                 Position fromPos,
                                 uint8_t fromStackPos,
                                 uint32_t creatureId,
                                 uint16_t spriteId,
                                 bool isHotkey)
{
	...
	if (!player->canDoAction()) {
		uint32_t delay = player->getNextActionTime();
		SchedulerTask *task =
		createSchedulerTask(delay, boost::bind(&Game::playerUseBattleWindow, this, playerId, fromPos,
		                                       fromStackPos, creatureId, spriteId, isHotkey));
		player->setNextActionTask(task);
		return false;
	}
	...
	ReturnValue ret = internalUseItemEx(player, PositionEx(fromPos, fromStackPos),
	                                    PositionEx(creature->getPosition(), 0), item,
	                                    targetCreature, NULL, isHotkey);
	player->setNextAction(OTSYS_TIME() + g_config.getNumber(ConfigManager::MIN_ACTIONEXTIME));
	...
	if (ret != RET_NOERROR) {
		if (ret == RET_NEEDTOPICKUPITEM) {
			...
			if (ret == RET_NOERROR) {
				...
				internalGetPosition(moveItem, fromPos, fromStackPos);
				ret = internalUseItemEx(player, PositionEx(fromPos, fromStackPos),
				                        PositionEx(creature->getPosition(), 0),
				                        item, targetCreature, NULL, isHotkey);
			}
		}
		...
		if (ret == RET_ITEMOUTORANGE) {
			...
			if (getPathToEx(player, fromPos, listDir, 0, 1, true, true)) {
				...
				SchedulerTask *task =
				createSchedulerTask(400, boost::bind(&Game::playerUseBattleWindow, this,
				                                     playerId, fromPos, fromStackPos,
...
bool Game::playerRotateItem(uint32_t playerId, const Position &pos, uint8_t stackPos, const uint16_t spriteId)
{
	...
	if (pos.x != 0xFFFF && !Position::areInRange<1, 1, 0>(pos, player->getPosition())) {
		...
		if (getPathToEx(player, pos, listDir, 0, 1, true, true)) {
			...
			SchedulerTask *task =
			createSchedulerTask(400, boost::bind(&Game::playerRotateItem, this,
...
bool Game::playerRequestTrade(uint32_t playerId, const Position &pos, uint8_t stackPos, uint32_t tradePlayerId, uint16_t spriteId)
{
	Player *player = getPlayerByID(playerId);
	if (!player || player->isRemoved()) return false;
	...
	Item *tradeItem =
	dynamic_cast<Item *>(internalGetThing(player, pos, stackPos, spriteId, STACKPOS_USE));
	if (!tradeItem || tradeItem->getClientID() != spriteId || !tradeItem->isPickupable() ||
	    !tradeItem->isTradeable()) {
		player->sendCancelMessage(RET_NOTPOSSIBLE);
		return false;
		...
		if (getPathToEx(player, pos, listDir, 0, 1, true, true)) {
			...
			SchedulerTask *task =
			createSchedulerTask(400, boost::bind(&Game::playerRequestTrade, this, playerId,
...
bool Game::internalStartTrade(Player *player, Player *tradePlayer, Item *tradeItem)
{
	if (player->tradeState != TRADE_NONE &&
	    !(player->tradeState == TRADE_ACKNOWLEDGE && player->tradePlayer == tradePlayer)) {
	...
	if (tradePlayer && tradePlayer->getTradeState() == TRADE_ACCEPT) {
		...
		if (ret1 == RET_NOERROR && ret2 == RET_NOERROR) {
			...
			if (ret1 == RET_NOERROR && ret2 == RET_NOERROR) {
				...
				internalMoveItem(NULL, cylinder1, tradePlayer, INDEX_WHEREEVER,
				                 tradeItem1, tradeItem1->getItemCount(), NULL);
				internalMoveItem(NULL, cylinder2, player, INDEX_WHEREEVER,
				                 tradeItem2, tradeItem2->getItemCount(), NULL);
				isCompleted = true;
			}
		}
...
bool Game::playerLookInTrade(uint32_t playerId, bool lookAtCounterOffer, int index)
{
	...
	Container *tradeContainer = tradeItem->getContainer();
	if (!tradeContainer || index > (int)tradeContainer->getItemHoldingCount()) return false;
...
bool Game::playerShopPurchase(uint32_t playerId, uint16_t spriteId, uint8_t count, uint8_t amount, bool ignoreCapacity, bool buyWithBackpack)
{
	Player *player = getPlayerByID(playerId);
...
bool Game::playerLookAt(uint32_t playerId, const Position &pos, uint16_t spriteId, uint8_t stackPos)
{
	...
	if (thing == player)
		...
		lookDistance =
		std::max(std::abs(playerPos.x - thingPos.x), std::abs(playerPos.y - thingPos.y));
		if (playerPos.z != thingPos.z) lookDistance = lookDistance + 9 + 6;
	}
	...
	if (script_system) {
		...
		if (script_system->dispatchEvent(evt)) return false;
	}
...
bool Game::playerRequestOutfit(uint32_t playerId)
{
	...
	*/
	...
	if (!outfitList.empty()) player->sendOutfitWindow(outfitList);
	return true;
}
...
bool Game::playerChangeOutfit(uint32_t playerId, OutfitType outfit)
{
	...
	if (!player->canWearOutfit(outfit)) return false;

	player->defaultOutfit = outfit;

	if (player->hasCondition(CONDITION_SHAPESHIFT)) return false;
...
bool Game::playerSay(uint32_t playerId, uint16_t channelId, SpeakClass type, std::string receiver, std::string text)
{
	Player *player = getPlayerByID(playerId);
	if (!player || player->isRemoved()) return false;

	if (checkReload(player, text)) return true;
	...
	if (isMuteableChannel) player->removeMessageBuffer();
	...
	case enums::SPEAK_CHANNEL_R1:
		return playerTalkToChannel(player, type, text, channelId) ||
		       playerSay(playerId, 0, SPEAK_SAY, receiver, text);
...
bool Game::playerWhisper(Player *player, const std::string &text)
{
	SpectatorVec list;
	SpectatorVec::iterator it;
	getSpectators(list, player->getPosition(), false, false, Map_maxClientViewportX,
	              Map_maxClientViewportX, Map_maxClientViewportY, Map_maxClientViewportY);
...
bool Game::playerSpeakTo(Player *player, SpeakClass type, const std::string &receiver, const std::string &text)
{
...
bool Game::playerTalkToChannel(Player *player, SpeakClass type, const std::string &text, unsigned short channelId)
{
	...
	// !player->hasFlag(PlayerFlag_CanTalkRedChannelAnonymous)){
	//  type = SPEAK_CHANNEL_Y;
	//}
...
bool Game::internalCreatureSay(Creature *creature, SpeakClass type, const std::string &text)
{
	...
	if (type == SPEAK_YELL || type == SPEAK_MONSTER_YELL) {
		...
		getSpectators(list, creature->getPosition(), false, false, Map_maxClientViewportX,
		              Map_maxClientViewportX, Map_maxClientViewportY, Map_maxClientViewportY);
	}
...
bool Game::getPathToEx(const Creature *creature,
...
bool Game::combatBlockHit(CombatType combatType,
                          ...
                          bool checkArmor)
{
	...
	if (blockType == BLOCK_DEFENSE) {
		...
		if (combatType == COMBAT_SCRIPTED_HEALTH)
			; // Nothing
		else if (combatType == COMBAT_ENERGYDAMAGE && combatType == COMBAT_FIREDAMAGE &&
...
bool Game::combatDamage(CombatType combatType, Creature *attacker, Creature *target, int32_t amount, bool showEffect /*= true*/)
{
...
bool Game::combatDamage(CombatType combatType, CombatSource combatSource, Creature *target, int32_t amount, bool showEffect /*= true*/)
{
	CombatEffect combatEffect(showEffect);
	return combatDamage(combatType, combatSource, combatEffect, target, amount);
}

bool Game::combatDamage(CombatType combatType, CombatSource combatSource, CombatEffect combatEffect, Creature *target, int32_t amount)
{
	// Don't send events for undefined damage
	if (combatType != COMBAT_SCRIPTED_HEALTH && combatType != COMBAT_SCRIPTED_MANA &&
	    g_game.onCreatureDamage(combatType, combatSource, target, amount)) {
		// handled by script
		return false;
	}
	...
	if (!target->isAttackable() || Combat::canDoCombat(combatSource.getSourceCreature(), target) != RET_NOERROR) {
		if (combatEffect.showEffect) addMagicEffect(list, targetPos, MAGIC_EFFECT_PUFF);
		return true;
	}
	...
	if (combatEffect.showEffect) {
		...
		if (combatEffect.hitEffect != MAGIC_EFFECT_UNK) hitEffect = combatEffect.hitEffect;
...
void Game::internalDecayItem(Item *item)
{
	...
	if (it.decayTo != 0) {
		...
		if (ret != RET_NOERROR) {
#ifdef __DEBUG__
			std::cout << "DEBUG, internalDecayItem failed, error code: " << ret
...
#ifdef __DEBUG__
		std::cout << "checkDecay: " << item << ", id:" << item->getID()
...
void Game::checkLight()
{
	...
	if (light_hour > 1440) light_hour = light_hour - 1440;
...
void Game::cleanup()
{
	...
	for (DecayList::iterator it = toDecayItems.begin(); it != toDecayItems.end(); ++it) {
		...
		if (dur >= EVENT_DECAYINTERVAL * EVENT_DECAY_BUCKETS) {
			...
			decayItems[(last_bucket + 1 + (*it)->getDuration() / 1000) % EVENT_DECAY_BUCKETS]
			.push_back(*it);
		}
	}
...
bool Game::playerViolationWindow(uint32_t playerId,
                                 ...
                                 bool ipBanishment)
{
	...
	if ((ipBanishment && ((violationNames[access] & Action_IpBan) != Action_IpBan ||
	                      (violationStatements[access] & Action_IpBan) != Action_IpBan)) ||
	    !(violationNames[access] & (1 << actionType) || violationStatements[access] & (1 << actionType)) ||
	    reasonId > violationReasons[access]) {
	...
	if (actionType == ACTION_NOTATION) {
		g_bans.addAccountNotation(account.number, player->getGUID(), comment, statement,
		                          reasonId, actionType);
		if (g_bans.getNotationsCount(account.number) >=
		    (uint32_t)g_config.getNumber(ConfigManager::NOTATIONS_TO_BAN)) {
			account.warnings++;
			if (account.warnings >= (uint32_t)g_config.getNumber(ConfigManager::WARNINGS_TO_DELETION)) {
				actionType = ACTION_DELETION;
				g_bans.addAccountBan(account.number, -1, player->getGUID(), comment,
				                     statement, reasonId, actionType);
			} else if (account.warnings >=
			           (uint32_t)g_config.getNumber(ConfigManager::WARNINGS_TO_FINALBAN)) {
				if (!g_bans.addAccountBan(account.number,
				                          (time(NULL) + g_config.getNumber(ConfigManager::FINALBAN_LENGTH)),
				                          player->getGUID(), comment, statement,
				                          reasonId, actionType))
					account.warnings--;
			} else {
				if (!g_bans.addAccountBan(account.number,
				                          (time(NULL) + g_config.getNumber(ConfigManager::BAN_LENGTH)),
				                          player->getGUID(), comment, statement,
				                          reasonId, actionType))
					account.warnings--;
			}
		...
		if (account.warnings++ >= (uint32_t)g_config.getNumber(ConfigManager::WARNINGS_TO_DELETION)) {
			actionType = ACTION_DELETION;
			if (!g_bans.addAccountBan(account.number, -1, player->getGUID(), comment,
			                          statement, reasonId, actionType))
				account.warnings--;
		} else if (g_bans.addAccountBan(account.number,
		                                (time(NULL) + g_config.getNumber(ConfigManager::FINALBAN_LENGTH)),
		                                player->getGUID(), comment, statement, reasonId, actionType)) {
	...
	if (ipBanishment && ip > 0)
		g_bans.addIpBan(ip, 0xFFFFFFFF,
		                (time(NULL) + g_config.getNumber(ConfigManager::IPBANISHMENT_LENGTH)),
		                player->getGUID(), comment);

	if (removeNotations > 1) g_bans.removeNotations(account.number);
	...
	if (actionType == ACTION_STATEMENT) {
		...
		ss << "  \"" << getViolationActionString(actionType, ipBanishment) << "\" "
		   ...
		   << "(Warnings: " << account.warnings << "), with reason: \""
		   << getViolationReasonString(reasonId) << "\", and comment: \"" << comment
		   << "\".";
	} else {
		if (announceViolation) {
			ss << player->getName() << " has taken the action";
		...
		ss << " \"" << getViolationActionString(actionType, ipBanishment)
		   << "\" against: " << targetName << " (Warnings: " << account.warnings
	...
	if (targetPlayer && removeNotations > 0) {
		...
		g_scheduler.addEvent(createSchedulerTask(1000, boost::bind(&Game::kickPlayer, this, targetId)));
	}
...
bool Game::playerReportViolation(uint32_t playerId,
                                 std::string violatorName,
                                 uint32_t reportType,
                                 uint32_t ruleViolation,
                                 std::string comment,
                                 ...
                                 uint32_t counter)
{
	...
	if (!violator || violator->isRemoved()) return false;
	...
	Statement/Bot Related
	 ...
	 5 ->  "repeating identical or similar statements within a short period of
	time"
...
bool Game::playerReportBug(uint32_t playerId, std::string comment)
{
	...
	if (of) {
		...
		of << "--------------------------------------------------------------------"
		      "-----------"
		   << std::endl
		   << today << " - " << player->getName() << "[x:" << pos.x << " y:" << pos.y
		   << " z:" << pos.z << "]" << std::endl
