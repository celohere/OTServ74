...
{
	if (isHouseTile()) return static_cast<HouseTile *>(this);
	return NULL;
}

const HouseTile *Tile::getHouseTile() const
{
	if (isHouseTile()) return static_cast<const HouseTile *>(this);
	return NULL;
}
...
{
	switch (zt.value()) {
	case enums::ZONE_PROTECTION:
		return hasFlag(TILEPROP_PROTECTIONZONE);
	case enums::ZONE_NOPVP:
		return hasFlag(TILEPROP_NOPVPZONE);
	case enums::ZONE_PVP:
		return hasFlag(TILEPROP_PVPZONE);
	case enums::ZONE_NOLOGOUT:
		return hasFlag(TILEPROP_NOLOGOUT);
	case enums::ZONE_NORMAL:
		return true;
	}
	return true;
}
...
void Tile::moveCreature(Creature *actor, Creature *creature, Cylinder *toCylinder, bool teleport /* = false*/)
{
	...
	for (it = list.begin(); it != list.end(); ++it) {
		if ((tmpPlayer = (*it)->getPlayer())) {
			tmpPlayer->sendCreatureMove(creature, newTile, newPos, this, oldPos,
			                            oldStackPosVector[i], teleport);
			++i;
		}
	}
...
ReturnValue Tile::__queryAdd(int32_t index, const Thing *thing, uint32_t count, uint32_t flags) const
{
	if (const Creature *creature = thing->getCreature()) {
		...
		if (const Actor *monster = creature->getActor()) {
			if (hasFlag(TILEPROP_PROTECTIONZONE)) return RET_NOTPOSSIBLE;
			...
			if (!hasBitSet(FLAG_IGNOREBLOCKCREATURE, flags)) {
				if (monster->canPushCreatures() && !monster->isSummon()) {
					...
					for (uint32_t i = 0; i < creatures_count(); ++i) {
						...
						if (!creature->getActor() || !creature->isPushable() ||
						    (creature->getActor()->isPlayerSummon())) {
							return RET_NOTPOSSIBLE;
						}
					}
				} else if (!creatures_empty()) {
					for (CreatureConstIterator cit = creatures_begin();
					     cit != creatures_end(); ++cit) {
			...
			if (hasBitSet(FLAG_PATHFINDING, flags) &&
			    (hasFlag(TILEPROP_BLOCKPATHNOTFIELD) && hasFlag(TILEPROP_BLOCKPATHNOTMOVEABLE))) {
				return RET_NOTPOSSIBLE;
			}

			if (hasFlag(TILEPROP_BLOCKSOLID) ||
			    (hasBitSet(FLAG_PATHFINDING, flags) && hasFlag(TILEPROP_BLOCKPATHNOTFIELD))) {
			...
			if (player->isPzLocked() && !player->getParentTile()->hasFlag(TILEPROP_PVPZONE) &&
			    hasFlag(TILEPROP_PVPZONE)) {
		...
		if (item->getParent() == NULL) {
			// The item has just been created and added to this tile through
			// Game::internalAddItem
			return RET_NOERROR;
		}
...
Cylinder *Tile::__queryDestination(int32_t &index, const Thing *thing, Item **destItem, uint32_t &flags)
{
	...
	*destItem = NULL;
	...
	if (floorChangeDown()) {
		...
		if (downTile) {
			if (downTile->floorChange(NORTH)) dy += 1;
			if (downTile->floorChange(SOUTH)) dy -= 1;
			if (downTile->floorChange(EAST)) dx -= 1;
			if (downTile->floorChange(WEST)) dx += 1;
		...
		if (floorChange(NORTH)) dy -= 1;
		if (floorChange(SOUTH)) dy += 1;
		if (floorChange(EAST)) dx += 1;
		if (floorChange(WEST)) dx -= 1;
		destTile = g_game.getParentTile(dx, dy, dz);
	}
			...
			if (item->isSplash()) {
				...
				for (TileItemIterator it = items_topBegin(); it != items_topEnd(); ++it) {
					if ((*it)->isSplash()) {
						...
						postRemoveNotification(actor, oldSplash, NULL,
						                       oldSplashIndex, true);
						break;
					}
				}
			...
			if (item->isMagicField()) {
				...
				if (oldField) {
					if (oldField->isReplaceable()) {
						...
						postRemoveNotification(actor, oldField, NULL,
						                       oldFieldIndex, true);
					} else {
		...
		if (item->isAlwaysOnTop()) {
			for (TileItemIterator it = items_topBegin(); it != items_topEnd(); ++it) {
				if (*it == item) {
					...
					Player *tmpPlayer = NULL;
					for (SpectatorVec::const_iterator iit = list.begin();
					     iit != list.end(); ++iit) {
						if ((tmpPlayer = (*iit)->getPlayer())) {
							oldStackPosVector.push_back(
							getClientIndexOfThing(tmpPlayer, *it));
						}
					}
					(*it)->setParent(NULL);
					items_erase(it);
			...
			for (TileItemIterator it = items_downBegin(); it != items_downEnd(); ++it) {
				if ((*it) == item) {
					if (item->isStackable() && count != item->getItemCount()) {
						...
						g_game.getSpectators(getPosition());
						std::vector<uint32_t> oldStackPosVector;
...
Thing *Tile::__getThing(uint32_t index) const
{
	...
	if (index < topItemSize) return items_get(items_downCount() + index);

	index -= topItemSize;

	if (index < (uint32_t)creatures_count()) return creatures_get(index);

	index -= (uint32_t)creatures_count();

	if (index < items_downCount()) return items_get(index);
...
void Tile::postAddNotification(Creature *actor, Thing *thing, const Cylinder *oldParent, int32_t index, cylinderlink_t link /*= LINK_OWNER*/)
{
	...
	// add a reference to this item, it may be deleted after being added
	// (trashholder for example)
	thing->addRef();
...
void Tile::postRemoveNotification(Creature *actor,
                                  Thing *thing,
                                  ...
                                  cylinderlink_t link /*= LINK_OWNER*/)
{
	...
	for (it = list.begin(); it != list.end(); ++it) {
		if ((tmpPlayer = (*it)->getPlayer())) {
			tmpPlayer->postRemoveNotification(actor, thing, newParent, index,
			                                  isCompleteRemoval, LINK_NEAR);
		}
	}
...
bool Tile::hasItemWithProperty(uint32_t props) const
{
	...
	for (TileItemConstIterator it = items_begin(); it != items_end(); ++it) {
		if ((*it)->hasProperty(props)) return true;
	}
...
bool Tile::hasItemWithProperty(Item *exclude, uint32_t props) const
{
	...
	for (TileItemConstIterator it = items_begin(); it != items_end(); ++it) {
		const Item *item = *it;
		if (item != exclude && item->hasProperty(props)) return true;
	}
...
void Tile::updateTileFlags(Item *item, bool removed)
{
	if (!removed) {
		...
		if (item->hasProperty(ITEMPROP_FLOORCHANGEDOWN) &&
		    !hasItemWithProperty(item, ITEMPROP_FLOORCHANGEDOWN)) {
			resetFlag(TILEPROP_FLOORCHANGE);
			resetFlag(TILEPROP_FLOORCHANGE_DOWN);
		}
		if (item->hasProperty(ITEMPROP_FLOORCHANGENORTH) &&
		    !hasItemWithProperty(item, ITEMPROP_FLOORCHANGENORTH)) {
			resetFlag(TILEPROP_FLOORCHANGE);
			resetFlag(TILEPROP_FLOORCHANGE_NORTH);
		}
		if (item->hasProperty(ITEMPROP_FLOORCHANGESOUTH) &&
		    !hasItemWithProperty(item, ITEMPROP_FLOORCHANGESOUTH)) {
			resetFlag(TILEPROP_FLOORCHANGE);
			resetFlag(TILEPROP_FLOORCHANGE_SOUTH);
		}
		if (item->hasProperty(ITEMPROP_FLOORCHANGEEAST) &&
		    !hasItemWithProperty(item, ITEMPROP_FLOORCHANGEEAST)) {
			resetFlag(TILEPROP_FLOORCHANGE);
			resetFlag(TILEPROP_FLOORCHANGE_EAST);
		}
		if (item->hasProperty(ITEMPROP_FLOORCHANGEWEST) &&
		    !hasItemWithProperty(item, ITEMPROP_FLOORCHANGEWEST)) {
			resetFlag(TILEPROP_FLOORCHANGE);
		...
		if (item->hasProperty(ITEMPROP_BLOCKPATHFIND) &&
		    !hasItemWithProperty(item, ITEMPROP_BLOCKPATHFIND)) {
		...
		if (item->hasProperty(ITEMPROP_BLOCKPROJECTILE) &&
		    !hasItemWithProperty(item, ITEMPROP_BLOCKPROJECTILE)) {
