...
{
	return countSpecificListeners() + Generic.OnSay.size() + Generic.OnUseItem.size() +
	       Generic.OnUseWeapon.size() + Generic.OnUseFist.size() + Generic.OnUseAnyWeapon.size() +
	       Generic.OnEquipItem.size() + Generic.OnMoveInAnyCreature.size() +
	       Generic.OnMoveOutAnyCreature.size() + Generic.OnMoveInCreature.size() +
	       Generic.OnMoveOutCreature.size() + Generic.OnJoinChannel.size() +
	       Generic.OnLeaveChannel.size() + Generic.OnAccountLogin.size() + Generic.OnLogin.size() +
	       Generic.OnLogout.size() + Generic.OnChangeOutfit.size() + Generic.OnLook.size() +
	       Generic.OnTurn.size() + Generic.OnLoad.size() + Generic.OnUnload.size() +
	       Generic.OnSpawn.size() + Generic.OnAdvance.size() + Generic.OnTrade.size() +
	       Generic.OnShopPurchase.size() + Generic.OnShopSell.size() + Generic.OnShopClose.size() +
	       Generic.OnTradeBegin.size() + Generic.OnTradeEnd.size() + Generic.OnMoveItem.size() +
	       Generic.OnMoveItemOnItem.size() + Generic.OnConditionEffect.size() +
	       Generic.OnAttack.size() + Generic.OnDamage.size() + Generic.OnKilled.size() +
	       Generic.OnKill.size() + Generic.OnDeathBy.size() + Generic.OnDeath.size() +
	       Generic.OnActorLoadSpell.size() + Generic.OnActorCastSpell.size();
}
...
{
	for (ListenerList::iterator giter = list.begin(), gend = list.end(); giter != gend; ++giter) {
...
bool Environment::stopListener(ListenerStringMap &list, uint32_t id)
{
	...
	for (ListenerStringMap::iterator giter = list.begin(), gend = list.end(); giter != gend; ++giter)
		if (stopListener(giter->second, id)) return true;
	return false;
}
...
bool Environment::stopListener(ListenerItemMap &item_map, uint32_t id)
{
	...
	for (list_iter = item_map.ItemId.begin(); list_iter != item_map.ItemId.end(); ++list_iter)
		if (stopListener(list_iter->second, id)) return true;
...
bool Environment::stopListener(ListenerType type, uint32_t id)
{
	...
	case enums::ON_USE_ITEM_LISTENER:
		if (stopListener(Generic.OnUseItem, id)) return true;
		break;
	case enums::ON_USE_WEAPON_LISTENER:
		if (stopListener(Generic.OnUseWeapon, id) ||
		    stopListener(Generic.OnUseAnyWeapon, id) || stopListener(Generic.OnUseFist, id))
			return true;
		break;
	case enums::ON_EQUIP_ITEM_LISTENER:
		if (stopListener(Generic.OnEquipItem, id)) return true;
		break;
	case enums::ON_MOVE_CREATURE_LISTENER:
		if (stopListener(Generic.OnMoveInAnyCreature, id) ||
		    stopListener(Generic.OnMoveInCreature, id) ||
		    stopListener(Generic.OnMoveOutAnyCreature, id) ||
		    stopListener(Generic.OnMoveOutCreature, id))
			return true;
		break;
	case enums::ON_MOVE_ITEM_LISTENER:
		if (stopListener(Generic.OnMoveItem, id)) return true;
		break;
	case enums::ON_OPEN_CHANNEL_LISTENER:
		if (stopListener(Generic.OnJoinChannel, id)) return true;
		break;
	case enums::ON_CLOSE_CHANNEL_LISTENER:
		if (stopListener(Generic.OnLeaveChannel, id)) return true;
		break;
	case enums::ON_ACCOUNT_LOGIN_LISTENER:
		if (stopListener(Generic.OnAccountLogin, id)) return true;
	case enums::ON_LOGIN_LISTENER:
		if (stopListener(Generic.OnLogin, id)) return true;
		return false; // No specific listeners
	case enums::ON_LOGOUT_LISTENER:
		if (stopListener(Generic.OnLogout, id)) return true;
		break;
	case enums::ON_CHANGE_OUTFIT_LISTENER:
		if (stopListener(Generic.OnChangeOutfit, id)) return true;
		break;
	case enums::ON_TURN_LISTENER:
		if (stopListener(Generic.OnTurn, id)) return true;
		break;
	case enums::ON_SPAWN_LISTENER:
		if (stopListener(Generic.OnSpawn, id)) return true;
		return false; // No specific listeners
	case enums::ON_ADVANCE_LISTENER:
		if (stopListener(Generic.OnAdvance, id)) return true;
	case enums::ON_SHOP_PURCHASE_LISTENER:
		if (stopListener(Generic.OnShopPurchase, id)) return true;
		return false; // No specific listeners
	case enums::ON_SHOP_SELL_LISTENER:
		if (stopListener(Generic.OnShopSell, id)) return true;
		return false; // No specific listeners
	case enums::ON_SHOP_CLOSE_LISTENER:
		if (stopListener(Generic.OnShopClose, id)) return true;
		return false; // No specific listeners
	case enums::ON_TRADE_BEGIN_LISTENER:
		if (stopListener(Generic.OnTradeBegin, id)) return true;
	case enums::ON_TRADE_END_LISTENER:
		if (stopListener(Generic.OnTradeEnd, id)) return true;
	case enums::ON_ATTACK_LISTENER:
		if (stopListener(Generic.OnAttack, id)) return true;
	case enums::ON_DAMAGE_LISTENER:
		if (stopListener(Generic.OnDamage, id)) return true;
	case enums::ON_KILLED_LISTENER:
		if (stopListener(Generic.OnKilled, id)) return true;
	case enums::ON_KILL_LISTENER:
		if (stopListener(Generic.OnKill, id)) return true;
	case enums::ON_DEATH_BY_LISTENER:
		if (stopListener(Generic.OnDeathBy, id)) return true;
	case enums::ON_DEATH_LISTENER:
		if (stopListener(Generic.OnDeath, id)) return true;
	case enums::ON_ACTOR_LOAD_SPELL_LISTENER:
		if (stopListener(Generic.OnActorLoadSpell, id)) return true;
	case enums::ON_ACTOR_CAST_SPELL_LISTENER:
		if (stopListener(Generic.OnActorCastSpell, id)) return true;
	default:
		break;
	}
