...
bool Event::call(Manager &state, Environment &environment, Listener_ptr listener)
{
	...
	if (thread->rawtype() == LUA_TNIL) {
		...
		thread->HandleError("Invalid type of value 'skipped' of event table from '" +
		                    getName() + "' listener.");
		thread->pop();
	}
...
OnServerLoad::Event::Event(bool real_startup) : real_startup(real_startup)
{
...
{
	return dispatchEvent<OnServerLoad::Event>(this, state, environment, environment.Generic.OnLoad);
}
...
OnServerUnload::Event::Event(bool real_shutdown) : real_shutdown(real_shutdown)
{
	propagate_by_default = true;
}
...
{
	return dispatchEvent<OnServerUnload::Event>(this, state, environment, environment.Generic.OnUnload);
}
...
: speaker(_speaker), speak_class(_class), channel(channel), text(_text)
{
}
...
bool OnSay::Event::dispatch(Manager &state, Environment &environment)
{
	...
	if (dispatchEvent<OnSay::Event, ScriptInformation>(this, state, environment, list))
		return true;

	return dispatchEvent<OnSay::Event, ScriptInformation>(this, state, environment,
	                                                      environment.Generic.OnSay);
}
...
: creature(creature), talking_creature(talking_creature), message(message), speak_class(speak_class)
{
}
...
bool OnHear::Event::dispatch(Manager &state, Environment &environment)
{
	...
	if (dispatchEvent<OnHear::Event>(this, state, environment, list)) return true;
	return false;
}
...
: user(user), item(item), targetPos(toPos), targetCreature(targetCreature), targetItem(targetItem),
  retval(retval)
{
	propagate_by_default = true;
}
...
: user(user), item(item), targetPos(NULL), targetCreature(NULL), targetItem(NULL), retval(retval)
{
	propagate_by_default = true;
}
...
bool OnUseItem::Event::dispatch(Manager &state, Environment &environment)
{
	...
	if (item->getActionId() != 0) {
		...
		if (list_iter != environment.Generic.OnUseItem.ActionId.end()) {
			if (dispatchEvent<OnUseItem::Event, ScriptInformation>(this, state, environment,
			                                                       list_iter->second)) {
				return true;
			}
		}
...
: player(player), attacked(attacked), weapon(weapon)
{
...
bool OnUseWeapon::Event::dispatch(Manager &state, Environment &environment)
{
	...
	if (!weapon) {
		if (dispatchEvent<OnUseWeapon::Event>(this, state, environment, environment.Generic.OnUseFist))
			return true;
	} else {
		if (weapon->getActionId() != 0) {
			...
			if (list_iter != environment.Generic.OnUseWeapon.ActionId.end()) {
				if (dispatchEvent<OnUseWeapon::Event>(this, state, environment,
				                                      list_iter->second)) {
					return true;
				}
			}
		...
		if (list_iter != environment.Generic.OnUseWeapon.ItemId.end()) {
			if (dispatchEvent<OnUseWeapon::Event>(this, state, environment, list_iter->second)) {
				return true;
			}
		}
	}

	return dispatchEvent<OnUseWeapon::Event>(this, state, environment, environment.Generic.OnUseAnyWeapon);
}
...
: user(user), item(item), equip(equip), postEvent(true), retval(dummyRetVal)
{
	propagate_by_default = true;

	switch (slot.value()) {
	case ::enums::SLOT_HEAD:
		slotPos = SLOTPOSITION_HEAD;
		break;
	case ::enums::SLOT_NECKLACE:
		slotPos = SLOTPOSITION_NECKLACE;
		break;
	case ::enums::SLOT_BACKPACK:
		slotPos = SLOTPOSITION_BACKPACK;
		break;
	case ::enums::SLOT_ARMOR:
		slotPos = SLOTPOSITION_ARMOR;
		break;
	case ::enums::SLOT_RIGHT:
		slotPos = SLOTPOSITION_RIGHT;
		break;
	case ::enums::SLOT_LEFT:
		slotPos = SLOTPOSITION_LEFT;
		break;
	case ::enums::SLOT_LEGS:
		slotPos = SLOTPOSITION_LEGS;
		break;
	case ::enums::SLOT_FEET:
		slotPos = SLOTPOSITION_FEET;
		break;
	case ::enums::SLOT_RING:
		slotPos = SLOTPOSITION_RING;
		break;
	case ::enums::SLOT_AMMO:
		slotPos = SLOTPOSITION_AMMO;
		break;
	default:
		slotPos = SLOTPOSITION_NONE;
		break;
	}
}

OnEquipItem::Event::Event(Player *user, Item *item, SlotType slot, bool equip, ReturnValue &retval)
: user(user), item(item), equip(equip), postEvent(false), retval(retval)
{
	propagate_by_default = true;
...
bool OnEquipItem::Event::dispatch(Manager &state, Environment &environment)
{
	...
	return dispatchEvent<OnEquipItem::Event, ScriptInformation>(this, state, environment,
	                                                            environment.Generic.OnEquipItem);
}
...
: actor(actor), moving_creature(moving_creature), fromTile(fromTile), toTile(toTile), item(NULL),
  moveType(TYPE_NONE)
{
	if (fromTile && toTile) {
		moveType = TYPE_MOVE;
...
bool OnMoveCreature::Event::dispatch(Manager &state, Environment &environment)
{
	...
	if (dispatchEvent<OnMoveCreature::Event, ScriptInformation>(this, state, environment, list))
		return true;
	...
	if (moveType == TYPE_MOVE || moveType == TYPE_STEPOUT) {
		...
		if (!id_list.empty()) {
			for (std::set<int32_t>::const_iterator id = id_list.begin();
			     id != id_list.end(); ++id) {
				...
				if (list_iter != environment.Generic.OnMoveOutCreature.ActionId.end()) {
					...
					if (dispatchEvent<OnMoveCreature::Event>(this, state, environment,
					                                         list_iter->second)) {
						return true;
					}
					item = NULL;
		...
		if (dispatchEvent<OnMoveCreature::Event, ScriptInformation>(this, state, environment,
		                                                            environment.Generic.OnMoveOutAnyCreature)) {
			return true;
		}
	}
	...
	if (moveType == TYPE_MOVE || moveType == TYPE_STEPIN) {
		...
		// Check generic matches
		if (dispatchEvent<OnMoveCreature::Event, ScriptInformation>(this, state, environment,
		                                                            environment.Generic.OnMoveInAnyCreature)) {
			return true;
		}
	}
...
OnTurn::Event::Event(Creature *creature, Direction dir) : creature(creature), direction(dir)
{
	propagate_by_default = true;
}
...
bool OnTurn::Event::dispatch(Manager &state, Environment &environment)
{
	...
	if (dispatchEvent<OnTurn::Event>(this, state, environment, list)) return true;

	return dispatchEvent<OnTurn::Event>(this, state, environment, environment.Generic.OnTurn);
}
...
: actor(actor), item(item), tile(tile), addItem(addItem), postEvent(true), retval(dummyRetVal)
{
	propagate_by_default = true;
}

OnMoveItem::Event::Event(Creature *actor, Item *item, Tile *tile, bool addItem, ReturnValue &retval)
: actor(actor), item(item), tile(tile), addItem(addItem), postEvent(false), retval(retval)
{
	propagate_by_default = true;
}
...
bool OnMoveItem::Event::dispatch(Manager &state, Environment &environment)
{
	...
	if (item->getActionId() != 0) {
		...
		if (list_iter != environment.Generic.OnMoveItem.ActionId.end()) {
			if (dispatchEvent<OnMoveItem::Event, ScriptInformation>(this, state, environment,
			                                                        list_iter->second)) {
				return true;
			}
		}
	...
	if (dispatchEvent<OnMoveItem::Event, ScriptInformation>(this, state, environment,
	                                                        environment.Generic.OnMoveItemOnItem)) {
		return true;
	}
...
bool OnJoinChannel::Event::dispatch(Manager &state, Environment &environment)
{
	...
	if (dispatchEvent<OnJoinChannel::Event>(this, state, environment, list)) return true;
	return dispatchEvent<OnJoinChannel::Event>(this, state, environment, environment.Generic.OnJoinChannel);
}
...
bool OnLeaveChannel::Event::dispatch(Manager &state, Environment &environment)
{
	...
	if (dispatchEvent<OnLeaveChannel::Event>(this, state, environment, list)) return true;
	return dispatchEvent<OnLeaveChannel::Event>(this, state, environment, environment.Generic.OnLeaveChannel);
}
...
OnAccountLogin::Event::Event(std::string &name,
                             uint32_t &number,
                             std::string &password,
                             time_t &premiumEnd,
                             uint32_t &warnings,
                             std::list<AccountCharacter> &charList)
: name(name), number(number), password(password), premiumEnd(premiumEnd), warnings(warnings),
  charList(charList)
{
	propagate_by_default = true;
}
...
{
	return dispatchEvent<OnAccountLogin::Event>(this, state, environment, environment.Generic.OnAccountLogin);
}
...
void OnAccountLogin::Event::push_instance(LuaState &state, Environment &environment)
{
	...
	state.newTable();
	int n = 1;
	for (std::list<AccountCharacter>::iterator iter = charList.begin(); iter != charList.end();
	     ++iter, ++n) {
		state.newTable();
...
void OnAccountLogin::Event::update_instance(Manager &state, Environment &environment, LuaThread_ptr thread)
{
	...
	if (thread->isString()) {
		...
		thread->HandleError(ERROR_WARN,
		                    "Event 'OnAccountLogin' invalid value of 'password'");
		thread->pop();
	}
	...
	if (thread->isNumber()) {
		...
		thread->HandleError(ERROR_WARN,
		                    "Event 'OnAccountLogin' invalid value of 'premiumEnd'");
		thread->pop();
	}
	...
	if (thread->isNumber()) {
		...
		thread->HandleError(ERROR_WARN,
		                    "Event 'OnAccountLogin' invalid value of 'warnings'");
		thread->pop();
	}

	try {
		thread->getField(-1, "charList");
		if (!thread->isTable()) thread->HandleError("'charList' must be a table.");
		...
		thread->HandleError(
		ERROR_WARN,
		std::string("Event 'OnAccountLogin' invalid structure of 'charList'") + e.what());
	}
}
...
OnLogin::Event::Event(Player *player) : player(player)
{
	propagate_by_default = true;
}
...
{
	return dispatchEvent<OnLogin::Event>(this, state, environment, environment.Generic.OnLogin);
}
...
: player(player), forced(forced), timeout(timeout)
{
	propagate_by_default = true;
}
...
bool OnLogout::Event::dispatch(Manager &state, Environment &environment)
{
	...
	if (dispatchEvent<OnLogout::Event>(this, state, environment, list)) return true;
	return dispatchEvent<OnLogout::Event>(this, state, environment, environment.Generic.OnLogout);
}
...
: player(player), outfitList(outfitList)
{
	propagate_by_default = true;
}
...
bool OnChangeOutfit::Event::dispatch(Manager &state, Environment &environment)
{
	...
	if (dispatchEvent<OnChangeOutfit::Event>(this, state, environment, list)) return true;
	return dispatchEvent<OnChangeOutfit::Event>(this, state, environment, environment.Generic.OnChangeOutfit);
}
...
void OnChangeOutfit::Event::update_instance(Manager &state, Environment &environment, LuaThread_ptr thread)
{
	...
	*/
	try {
		...
		if (!thread->isTable()) thread->HandleError("'outfits' must be a table.");
		// iterate over the table
		thread->pushNil();
		while (thread->iterateTable(-2)) {
			Outfit o;
		...
		thread->HandleError(
		ERROR_WARN, std::string("Event 'OnChangeOutfit' invalid structure of 'outfits'") + e.what());
	}
}
...
: player(player), desc(desc), object(object)
{
	propagate_by_default = true;
}
...
bool OnLook::Event::dispatch(Manager &state, Environment &environment)
{
	...
	if (dispatchEvent<OnLook::Event>(this, state, environment, list)) return true;
	...
	if (item) {
		...
		if (item->getActionId() != 0) {
			...
			if (list_iter != environment.Generic.OnLook.ActionId.end()) {
				if (dispatchEvent<OnLook::Event, ScriptInformation>(this, state, environment,
				                                                    list_iter->second)) {
					return true;
				}
			}
...
: creature(creature), spotted_creature(spotted_creature)
{
}
...
bool OnSpotCreature::Event::dispatch(Manager &state, Environment &environment)
{
	...
	if (dispatchEvent<OnSpotCreature::Event>(this, state, environment, list)) return true;
	return true;
}
...
: creature(creature), lose_creature(lose_creature)
{
}
...
bool OnLoseCreature::Event::dispatch(Manager &state, Environment &environment)
{
	...
	if (dispatchEvent<OnLoseCreature::Event>(this, state, environment, list)) return true;
	return true;
}
...
OnSpawn::Event::Event(Actor *actor, bool reloading) : actor(actor), reloading(reloading)
{
	propagate_by_default = true;
}
...
{
	Script::ListenerStringMap::iterator eiter =
	environment.Generic.OnSpawn.find(asLowerCaseString(actor->getName()));

	if (eiter != environment.Generic.OnSpawn.end())
		return dispatchEvent<OnSpawn::Event>(this, state, environment, eiter->second);
	return false;
}
...
OnThink::Event::Event(Creature *creature, int32_t interval) : creature(creature), interval(interval)
{
	propagate_by_default = true;
}
...
bool OnThink::Event::dispatch(Manager &state, Environment &environment)
{
	...
	if (dispatchEvent<OnThink::Event>(this, state, environment, list)) return true;
	return false;
}
...
: player(_player), skill(_skill), oldSkillLevel(_oldSkillLevel), newSkillLevel(_newSkillLevel)
{
	propagate_by_default = true;
}
...
bool OnAdvance::Event::dispatch(Manager &state, Environment &environment)
{
	...
	if (dispatchEvent<OnAdvance::Event, ScriptInformation>(this, state, environment, list))
		return true;

	return dispatchEvent<OnAdvance::Event, ScriptInformation>(this, state, environment,
	                                                          environment.Generic.OnAdvance);
}
...
: player(player), itemId(itemId), type(type), amount(amount), ignoreCapacity(ignoreCapacity),
...
bool OnShopPurchase::Event::dispatch(Manager &state, Environment &environment)
{
	...
	if (dispatchEvent<OnShopPurchase::Event>(this, state, environment, list)) return true;

	return dispatchEvent<OnShopPurchase::Event>(this, state, environment, environment.Generic.OnShopPurchase);
}
...
: player(player), itemId(itemId), type(type), amount(amount)
{
	propagate_by_default = true;
}
...
bool OnShopSell::Event::dispatch(Manager &state, Environment &environment)
{
	...
	if (dispatchEvent<OnShopSell::Event>(this, state, environment, list)) return true;

	return dispatchEvent<OnShopSell::Event>(this, state, environment, environment.Generic.OnShopSell);
}
...
OnShopClose::Event::Event(Player *player) : player(player)
{
	propagate_by_default = true;
}
...
bool OnShopClose::Event::dispatch(Manager &state, Environment &environment)
{
	...
	if (dispatchEvent<OnShopClose::Event>(this, state, environment, list)) return true;

	return dispatchEvent<OnShopClose::Event>(this, state, environment, environment.Generic.OnShopClose);
}
...
: player1(player1), item1(item1), player2(player2), item2(item2)
{
	propagate_by_default = true;
}
...
bool OnTradeBegin::Event::check_match(const ScriptInformation &info)
{
	...
	case FILTER_ACTIONID:
		return (item1 && item1->getActionId() == info.id) ||
		       (item2 && item2->getActionId() == info.id);
	default:
		break;
	}
	return false;
...
bool OnTradeBegin::Event::dispatch(Manager &state, Environment &environment)
{
	if (player1) {
		...
		if (dispatchEvent<OnTradeBegin::Event, ScriptInformation>(this, state, environment, list))
			return true;
	}
	...
	return dispatchEvent<OnTradeBegin::Event>(this, state, environment, environment.Generic.OnTradeBegin);
}
...
: player1(player1), item1(item1), player2(player2), item2(item2), isCompleted(isCompleted)
{
	propagate_by_default = true;
}
...
bool OnTradeEnd::Event::dispatch(Manager &state, Environment &environment)
{
	if (player1) {
		...
		if (dispatchEvent<OnTradeEnd::Event>(this, state, environment, list)) return true;
	}
	...
	return dispatchEvent<OnTradeEnd::Event>(this, state, environment, environment.Generic.OnTradeEnd);
}
...
: creature(creature), effect(effect), eventType(EVENT_BEGIN)
{
	propagate_by_default = true;
}

OnConditionEffect::Event::Event(Creature *creature, ConditionEffect &effect, ConditionEnd reason)
: creature(creature), effect(effect), reason(reason), eventType(EVENT_END)
{
	propagate_by_default = true;
}

OnConditionEffect::Event::Event(Creature *creature, ConditionEffect &effect, uint32_t ticks)
: creature(creature), effect(effect), reason(CONDITIONEND_UPDATE), eventType(EVENT_TICK)
{
	propagate_by_default = true;
}
...
bool OnConditionEffect::Event::dispatch(Manager &state, Environment &environment)
{
	if (creature) {
		...
		if (dispatchEvent<OnConditionEffect::Event, ScriptInformation>(this, state, environment, list))
			return true;
	}

	if (dispatchEvent<OnConditionEffect::Event, ScriptInformation>(this, state, environment,
	                                                               environment.Generic.OnConditionEffect))
		return true;
...
: creature(creature), attacked(attacked)
{
	propagate_by_default = true;
}
...
bool OnAttack::Event::dispatch(Manager &state, Environment &environment)
{
	if (creature) {
		...
		if (dispatchEvent<OnAttack::Event, ScriptInformation>(this, state, environment, list))
			return true;
	}

	if (dispatchEvent<OnAttack::Event, ScriptInformation>(this, state, environment,
	                                                      environment.Generic.OnAttack))
		return true;
...
: combatType(combatType), combatSource(combatSource), creature(creature), value(value)
{
	propagate_by_default = true;
}
...
bool OnDamage::Event::dispatch(Manager &state, Environment &environment)
{
	if (creature) {
		...
		if (dispatchEvent<OnDamage::Event, ScriptInformation>(this, state, environment, list))
			return true;
	}

	if (dispatchEvent<OnDamage::Event, ScriptInformation>(this, state, environment,
	                                                      environment.Generic.OnDamage))
		return true;
...
void OnDamage::Event::update_instance(Manager &state, Environment &environment, LuaThread_ptr thread)
{
	...
	if (thread->isTable()) {
		try {
			...
			thread->HandleError(ERROR_WARN,
			                    "Event 'OnDamage' invalid value of 'combatType'");
			thread->pop();
		}
	} else {
...
: creature(creature), combatSource(combatSource)
{
	propagate_by_default = true;
}
...
bool OnKill::Event::dispatch(Manager &state, Environment &environment)
{
	...
	if (killer) {
		...
		if (dispatchEvent<OnKill::Event, ScriptInformation>(this, state, environment, list))
			return true;
	}

	if (dispatchEvent<OnKill::Event, ScriptInformation>(this, state, environment,
	                                                    environment.Generic.OnKill))
		return true;
...
: creature(creature), corpse(corpse), killer(killer)
{
	propagate_by_default = true;
}
...
bool OnDeath::Event::dispatch(Manager &state, Environment &environment)
{
	...
	if (killer) {
		...
		if (dispatchEvent<OnDeath::Event, ScriptInformation>(this, state, environment, list))
			return true;
	}

	if (dispatchEvent<OnDeath::Event, ScriptInformation>(this, state, environment,
	                                                     environment.Generic.OnDeathBy))
		return true;
...
OnActorLoadSpell::Event::Event(const SpellBlock &spell) : spell(spell)
{
	propagate_by_default = true;
}
...
{
	if (dispatchEvent<OnActorLoadSpell::Event>(this, state, environment, environment.Generic.OnActorLoadSpell))
		return true;
	return false;
}
...
void OnActorLoadSpell::Event::push_instance(LuaState &state, Environment &environment)
{
	...
	if (spell.condition.type != CONDITION_NONE) {
		...
		case ::enums::CONDITION_CURSED: {
			const ConditionEffect::ModPeriodicDamage &modPeriodicDamage =
			spell.condition.effect.getModEffect<const ConditionEffect::ModPeriodicDamage>();
		...
		case ::enums::CONDITION_HASTE: {
			const ConditionEffect::ModSpeed &modSpeed =
			spell.condition.effect.getModEffect<const ConditionEffect::ModSpeed>();
		...
		case ::enums::CONDITION_SHAPESHIFT: {
			const ConditionEffect::ModShapeShift &modShapeShift =
			spell.condition.effect.getModEffect<const ConditionEffect::ModShapeShift>();
...
: actor(actor), target(target), name(name)
{
	propagate_by_default = true;
}
...
{
	if (dispatchEvent<OnActorCastSpell::Event>(this, state, environment, environment.Generic.OnActorCastSpell))
		return true;
	return false;
}
