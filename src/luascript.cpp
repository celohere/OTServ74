...
void ScriptEnviroment::resetEnv()
{
	...
	for (auto& mit : m_tempItems) {
		...
		for (auto& it : itemList) {
			if (it->getParent() == nullptr) g_game.FreeThing(it);
		}
	}
...
void ScriptEnviroment::getEventInfo(int32_t& scriptId,
                                    std::string& desc,
...
uint32_t ScriptEnviroment::addThing(Thing* thing)
{
	if (thing && !thing->isRemoved()) {
		...
		if (Creature* creature = thing->getCreature()) {
			...
			++m_lastUID;
			if (m_lastUID > 0xFFFFFF) m_lastUID = 70000;
...
bool ScriptEnviroment::removeResult(uint32_t id)
{
	...
	if (it == m_tempResults.end()) return false;

	if (it->second) Database::instance()->freeResult(it->second);
...
void LuaScriptInterface::dumpLuaStack()
{
	...
	for (int i = 1; i <= a; ++i) {
		std::cout << lua_typename(m_luaState, lua_type(m_luaState, -i)) << " "
		          << lua_topointer(m_luaState, -i) << std::endl;
	}
}
...
bool LuaScriptInterface::initState()
{
	...
	if (loadFile(std::string(datadir + "global.lua")) == -1) {
		std::cout << "Warning: [LuaScriptInterface::initState] Can not load " << datadir
		          << "global.lua." << std::endl;
	}
...
bool LuaScriptInterface::closeState()
{
	if (m_luaState) {
		...
		for (it = m_timerEvents.begin(); it != m_timerEvents.end(); ++it) {
			for (std::list<int>::iterator lt = it->second.parameters.begin();
			     lt != it->second.parameters.end(); ++lt) {
...
void LuaScriptInterface::executeTimerEvent(uint32_t eventIndex)
{
	...
	if (it != m_timerEvents.end()) {
		...
		if (reserveScriptEnv()) {
			...
			std::cout
			<< "[Error] Call stack overflow. LuaScriptInterface::executeTimerEvent"
			<< std::endl;
		}
...
// getPlayerSoul(cid)
	...
	lua_register(m_luaState, "doErasePlayerStorageValueByName",
	             LuaScriptInterface::luaDoErasePlayerStorageValueByName);
...
// doPlayerAddSoul(cid, soul)
	...
	// doPlayerAddItem(cid, itemid, <optional: default: 1> count, <optional: default: 1>
	// canDropOnMap, <optional: default: 1>subtype)
	...
	// doRelocate(pos, posTo, <optional: default: false> moveUnmoveable, <optional: default: 0>
	// maxAmount)
	...
	// doPlayerSummonCreature(cid, name, pos, <optional> extendedPosition, <optional>
	// forceSpawn)
	...
	// doPlayerAddExp(cid, exp, <optional: default: 0> useRate, <optional: default: 0>
	// useMultiplier)
	lua_register(m_luaState, "doPlayerAddExp", LuaScriptInterface::luaDoPlayerAddExp);

	// doPlayerRemoveExp(cid, exp, <optional: default: 0> useRate, <optional: default: 0>
	// useMultiplier)
	...
	// addOutfitCondition(condition, lookTypeEx, lookType, lookHead, lookBody, lookLegs,
	// lookFeet)
	...
	// addBanishment(playerName = 0xFFFFFFFF[, length = 0[, admin = 0[, reason = "No reason"[,
	// comment = "No comment"]]])
	lua_register(m_luaState, "addBan", LuaScriptInterface::luaAddBan);

	// addIPBanishment(ip[, length = 0[, admin = 0[, reason = "No reason"[, comment = "No
	// comment"]]]])
	...
	// getCreatureConditionInfo(cid, conditionType, <optional: default: 0> subId, <optional:
	// default: CONDITIONID_DEFAULT> conditionId)
	lua_register(m_luaState, "getCreatureConditionInfo", LuaScriptInterface::luaGetCreatureConditionInfo);

	// getCreatureCondition(cid, conditionType, <optional: default: 0> subId, <optional:
	// default: CONDITIONID_DEFAULT> conditionId)
...
//}
	...
	{
		/*lua_newtable(L);
		setField(L, "day", IOPlayer::instance()->getPlayerUnjustKillCount(player,
		UNJUST_KILL_PERIOD_DAY) );
		setField(L, "dayRedSkull",
		g_config.getNumber(ConfigManager::KILLS_PER_DAY_RED_SKULL) );

		setField(L, "week", IOPlayer::instance()->getPlayerUnjustKillCount(player,
		UNJUST_KILL_PERIOD_WEEK));
		setField(L, "weekRedSkull",
		g_config.getNumber(ConfigManager::KILLS_PER_WEEK_RED_SKULL) );

		setField(L, "month", IOPlayer::instance()->getPlayerUnjustKillCount(player,
		UNJUST_KILL_PERIOD_MONTH));
		setField(L, "monthRedSkull",
		g_config.getNumber(ConfigManager::KILLS_PER_MONTH_RED_SKULL) );*/
		return 1;
	}
...
int LuaScriptInterface::luaPlayerLearnInstantSpell(lua_State* L)
{
	...
	if (!spell) {
		...
		(std::string) "Spell \"" + spellName + (std::string) "\" not found";
		reportErrorFunc(error_str);
		lua_pushboolean(L, false);
		return 1;
	}
...
int LuaScriptInterface::luaDoPlayerAddMana(lua_State* L)
{
	...
	if (lua_gettop(L) >= 3) withoutAnimation = popBoolean(L);
...
{
	// doPlayerAddItem(cid, itemid, <optional: default: 1> count/subtype, <optional: default:
	// true> canDropOnMap)
	// doPlayerAddItem(cid, itemid, <optional: default: 1> count, <optional: default: true>
	// canDropOnMap, <optional: default: 1>subtype)
	int32_t parameters = lua_gettop(L);
	...
	if (parameters > 4) {
		...
		if (it.stackable) itemCount = (int32_t)std::ceil((float)count / 100);
	...
	while (itemCount > 0) {
		...
		if (itemCount == 0) {
			if (newItem->getParent()) {
				...
				// stackable item stacked with existing object, newItem will be
				// released
				lua_pushnumber(L, 0);
				return 1;
			}
...
int LuaScriptInterface::luaGetThingDefaultDescription(lua_State* L)
{
	...
	if (thing) {
		...
		if (canSeeXRay) ret << thing->getXRayDescription();
...
{
	// Consider using getTileItemById/getTileItemByType/getTileThingByPos/getTopCreature
	// instead.
...
int LuaScriptInterface::luaGetAllCreatures(lua_State* L) // fix me
{
	...
	uint32_t flag =
	9; // by default shows visible monsters and players without any kind of invisibility flag
	...
	// if (creatures){
	// for(CreatureVector::const_iterator cit = creatures->begin(); cit != creatures->end();
	// ++cit){
	//	bool add = false;
	//	if(Player *p = (*cit)->getPlayer()) {
	//		if(((flag & 1) == 1 && !p->hasFlag(PlayerFlag_CannotBeSeen) &&
	//! p->isGmInvisible())||
	...
	//		if (((flag & 8) == 8 && !m->hasCondition(CONDITION_INVISIBLE)) || (flag &
	// 16)
	//== 16){
	//			add = true;
	//		}
...
{
	// doSummonCreature(name, pos, <optional> extendedPosition, <optional: default: false>
	// forceSpawn)
	...
	if (!monster) {
		std::string error_str =
		(std::string) "Monster name(" + name + (std::string) ") not found";
		reportErrorFunc(error_str);
		lua_pushboolean(L, false);
		return 1;
	}
...
{
	// doPlayerSummonCreature(cid, name, pos, <optional: default: false> extendedPosition,
	// <optional: default: false> forceSpawn)
	bool extentedPosition = false;
	bool forceSpawn = false;
	int32_t parameters = lua_gettop(L);
	if (parameters > 4) {
...
int LuaScriptInterface::luaGetPlayerItemCount(lua_State* L)
{
	...
	if (lua_gettop(L) > 2) subtype = popNumber(L);
...
int LuaScriptInterface::luaGetHouseList(lua_State* L)
{
	...
	if (lua_gettop(L) > 0) townid = popNumber(L);

	lua_newtable(L);
	uint32_t n = 1;
	for (HouseMap::iterator it = Houses::getInstance().getHouseBegin();
	     it != Houses::getInstance().getHouseEnd(); ++n, ++it) {
		House* house = it->second;

		if (townid != 0 && house->getTownId() != townid) continue;
		lua_pushnumber(L, n);
		lua_pushnumber(L, house->getHouseId());
		lua_settable(L, -3);
	}
...
int LuaScriptInterface::luaDoPlayerAddExp(lua_State* L)
{
	...
	if (player) {
		if (exp > 0) {
			exp =
			uint64_t(exp * (useMultiplier ? player->getRateValue(LEVEL_EXPERIENCE) : 1.0) *
...
int LuaScriptInterface::luaCreateCombatObject(lua_State* L)
{
	...
	if (env->getScriptId() != EVENT_ID_LOADING) {
		reportError(__FUNCTION__,
		            "This function can only be used while loading the script.");
		lua_pushboolean(L, false);
		return 1;
	}
...
bool LuaScriptInterface::getArea(lua_State* L, std::list<uint32_t>& list, uint32_t& rows)
{
	if (lua_type(L, -1) != LUA_TTABLE) {
		reportError(__FUNCTION__,
		            "Variable passed to [LuaInterface::getArea] is not a table.");
		return false;
	}
	rows = 0;
	uint32_t i = 0, j = 0;
...
int LuaScriptInterface::luaGetMonsterFriendList(lua_State* L)
{
	...
	for (CreatureList::const_iterator it = friendList.begin(); it != friendList.end(); ++it) {
		friendCreature = *it;
		if (!friendCreature->isRemoved() &&
		    friendCreature->getPosition().z == monster->getPosition().z) {
...
int LuaScriptInterface::luaDoMoveCreature(lua_State* L)
{
	...
	int32_t parameters = lua_gettop(L);
	if (parameters >= 3) flags = popNumber(L);
...
int LuaScriptInterface::luaIsCreatureImmuneToCondition(lua_State* L) // fix me
{
	...
	bool aggressive;
	int32_t parameters = lua_gettop(L);
	if (parameters >= 3) aggressive = popBoolean(L);
	ConditionType_t conditionType = (ConditionType_t)popNumber(L);
	if (parameters < 3) aggressive = Condition::canBeAggressive(conditionType);
	uint32_t cid = popNumber(L);
...
int LuaScriptInterface::luaDoSetCreatureLight(lua_State* L)
{
	...
	Creature* creature = env->getCreatureByUID(cid);
	if (creature) {
		Condition* condition = Condition::createCondition(CONDITIONID_COMBAT, CONDITION_LIGHT,
		                                                  time, level | (color << 8));
...
int LuaScriptInterface::luaAddEvent(lua_State* L)
{
	...
	int32_t parameters = lua_gettop(L);
	if (lua_isfunction(L, -parameters) ==
	    0) { //-parameters means the first parameter from left to right
	...
	std::list<int> params;
	for (int32_t i = 0; i < parameters - 2;
	...
	eventDesc.eventId = Scheduler::getScheduler().addEvent(
	createSchedulerTask(delay, boost::bind(&LuaScriptInterface::executeTimerEvent, script_interface,
	                                       script_interface->m_lastEventTimerId)));
...
int LuaScriptInterface::luaAddBan(lua_State* L)
{
	...
	if (parameters > 4) comment = popString(L);
	if (parameters > 3) reason = popString(L);
	if (parameters > 2) admin = popNumber(L);
...
int LuaScriptInterface::luaGetNpcParameterByName(lua_State* L) // fix me
{
	...
	for(AutoList<Npc>::listiterator it = Npc::listNpc.list.begin(); it !=
	Npc::listNpc.list.end(); ++it){
	        if(it->second->getName() == name){
	                npc = it->second;
	                break;
	        }
...
int LuaScriptInterface::luaGetCreatureConditionInfo(lua_State* L) // fix me
{
	...
	if (lua_gettop(L) > 3) conditionId = (ConditionId_t)popNumber(L);

	int subId = 0;
	if (lua_gettop(L) > 2) subId = popNumber(L);
