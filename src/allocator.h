...
template <typename T> class dummyallocator
{
public:
	...
	template <class U> dummyallocator(const dummyallocator<U> &) throw()
	{
	}
	~dummyallocator() throw()
	{
	...
	void *allocate(size_t size)
	{
		...
		for (it = pools.begin(); it != pools.end(); ++it) {
			if (it->first >= size + sizeof(poolTag)) {
				// poolTag* tag =
				// reinterpret_cast<poolTag*>(it->second->ordered_malloc());
	...
	void dumpStats()
	{
		...
		for (it = poolsStats.begin(); it != poolsStats.end(); ++it) {
			...
			if (it->second->allocations != 0 && it->first != 0) {
				output
				<< " avg: "
				<< (int64_t)((it->first) - (it->second->unused) / (it->second->allocations))
				<< " unused: " << (int64_t)((it->second->unused) * 100 /
				                            (it->second->allocations) / (it->first));
			}
			output
			<< "% N: " << ((int64_t)(it->second->allocations) - (int64_t)(it->second->deallocations))
			<< std::endl;
		}
		output << std::endl;
		output.close();
