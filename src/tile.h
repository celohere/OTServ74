...
// item id
boost::multi_index::ordered_non_unique<BOOST_MULTI_INDEX_CONST_MEM_FUN(Item, uint16_t, getID)>,

// action id
boost::multi_index::ordered_non_unique<BOOST_MULTI_INDEX_CONST_MEM_FUN(Item, int32_t, getActionId)>,

// item type
boost::multi_index::ordered_non_unique<BOOST_MULTI_INDEX_CONST_MEM_FUN(Item, ItemTypes_t, getType)>

// Notice: When adding new attributes, make sure items_onItemModified is
// called whenever you change
...
public:
	...
	const T_iter &getVectorPos() const
	{
		return vector_pos;
	}
	const E_iter &getMultiIndexPos() const
	{
		return multiIndex_pos;
	}
...
public:
	...
	{
		return const_cast<Item *>(*(items_begin() + _pos));
	}
	...
	TileItemIterator items_downBegin()
	{
		return items_begin();
	}
	TileItemConstIterator items_downBegin() const
	{
		return items_begin();
	}
	TileItemIterator items_downEnd()
	{
		return items_begin() + downItemCount;
	}
	TileItemConstIterator items_downEnd() const
	{
		return items_begin() + downItemCount;
	}
	TileItemIterator items_topBegin()
	{
		return items_begin() + downItemCount;
	}
	TileItemConstIterator items_topBegin() const
	{
		return items_begin() + downItemCount;
	}
	TileItemIterator items_topEnd()
	{
		return items_end();
	}
	TileItemConstIterator items_topEnd() const
	{
		return items_end();
	}
	uint32_t items_topCount() const
	{
		return std::distance(items_topBegin(), items_topEnd());
	}
	uint32_t items_downCount() const
	{
		return std::distance(items_downBegin(), items_downEnd());
	}
	Item *items_firstDown()
	{
		return *items_downBegin();
	}
	Item *items_firstTop()
	{
		return (items_topCount() > 0 ? *(items_topEnd() - 1) : NULL);
	}
	//
	...
	Creature *creatures_get(size_t _pos) const
	{
		return const_cast<Creature *>(*(creatures_begin() + _pos));
	}
	uint32_t creatures_count() const;
	bool creatures_empty() const
	{
		return creatures_count() == 0;
	}
	//
	...
	{
		return (ground ? 1 : 0) + items_count() + creatures_count();
	}
	...
	bool blockSolid() const
	{
		return hasFlag(TILEPROP_BLOCKSOLID);
	}
	bool blockPathFind() const
	{
		return hasFlag(TILEPROP_BLOCKPATH);
	}
	bool blockProjectile() const
	{
		return hasFlag(TILEPROP_BLOCKPROJECTILE);
	}
	bool isVertical() const
	{
		return hasFlag(TILEPROP_VERTICAL);
	}
	bool isHorizontal() const
	{
		return hasFlag(TILEPROP_HORIZONTAL);
	}

	bool hasFlag(TileProp flag) const
	{
		return ((m_flags & (uint32_t)flag.value()) == (uint32_t)flag.value());
	}
	void setFlag(TileProp flag)
	{
		m_flags |= (uint32_t)flag.value();
	}
	void resetFlag(TileProp flag)
	{
		m_flags &= ~(uint32_t)flag.value();
	}
	...
	{
		return hasFlag(TILEPROP_POSITIONCHANGE);
	}
	bool floorChange() const
	{
		return hasFlag(TILEPROP_FLOORCHANGE);
	}
	bool floorChangeDown() const
	{
		return hasFlag(TILEPROP_FLOORCHANGE_DOWN);
	}
	...
	// Should be called when itemId/actionId or any other data associated with
...
protected:
	...
	{
		return hasFlag(TILEPROP_DYNAMIC_TILE);
	}
	...
	void creatures_push_back(Creature *creature)
	{
...
public:
	...
	TileItemIterator items_begin()
	{
		return TileItemIterator(&items);
	}
	TileItemConstIterator items_begin() const
	{
		return TileItemConstIterator(&items);
	}
	TileItemIterator items_end()
	{
		return TileItemIterator(&items, items.end());
	}
	TileItemConstIterator items_end() const
	{
		return TileItemConstIterator(&items, items.end());
	}
	...
	ItemVector items_getListWithItemId(uint16_t itemId, int32_t max_result = -1) const
	{
		...
		for (TileItemConstIterator it = items_begin();
		     (it != items_end() && (max_result == -1 || (int32_t)vector.size() < max_result)); ++it) {
			if ((*it)->getID() == itemId) {
	...
	void items_fetchListWithActionId(std::set<int32_t> &into, int max_result /* = -1*/) const
	{
		...
		for (TileItemConstIterator it = items_begin();
		     (it != items_end() && (max_result == -1 || (int32_t)into.size() < max_result)); ++it) {
			if ((*it)->getActionId() != 0) {
...
public:
	...
	TileItemIterator items_begin()
	{
		return (items ? TileItemIterator(items) : TileItemIterator(&null_items));
	}
	TileItemConstIterator items_begin() const
	{
		return (items ? TileItemConstIterator(items) : TileItemConstIterator(&null_items));
	}
	TileItemIterator items_end()
	{
		return (items ? TileItemIterator(items, items->end()) :
		                TileItemIterator(&null_items, null_items.end()));
	}
	TileItemConstIterator items_end() const
	{
		return (items ? TileItemConstIterator(items, items->end()) :
		                TileItemConstIterator(&null_items, null_items.end()));
	}
	...
	uint32_t items_count() const
	{
		return (items ? (uint32_t)items->size() : 0);
	}

	CreatureIterator creatures_begin()
	{
		return (creatures ? creatures->begin() : null_creatures.begin());
	}
	CreatureConstIterator creatures_begin() const
	{
		return (creatures ? creatures->begin() : null_creatures.begin());
	...
	CreatureIterator creatures_end()
	{
		return (creatures ? creatures->end() : null_creatures.end());
	}
	CreatureConstIterator creatures_end() const
	{
		return (creatures ? creatures->end() : null_creatures.end());
...
public:
	...
	ItemVector items_getListWithProps(ItemProp props, int32_t max_result = -1) const
	{
		...
		for (TileItemConstIterator it = items_begin();
		     (it != items_end() && (max_result == -1 || max_result < (int32_t)vector.size())); ++it) {
			if ((*it)->hasProperty(props)) {
				vector.push_back(const_cast<Item *>(*it));
			}
		}
...
protected:
	...
	// Basically, we call NoOperationFunctor to make the MultiIndex think we are
	// modifying the item
...
: qt_node(NULL), ground(NULL), downItemCount(0), tilePos(x, y, z), m_flags(0)
{
}
...
inline IndexedTile::IndexedTile(uint16_t x, uint16_t y, uint16_t z) : Tile(x, y, z)
{
