...
/// "getInstance()" is called for the first time, avoiding initialization
/// fiasco.
...
template <typename T> boost::scoped_ptr<T> Singleton<T>::m_pointer;

template <typename T> boost::once_flag Singleton<T>::m_flag = BOOST_ONCE_INIT;
