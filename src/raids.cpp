		...
		while (raidNode) {
			if (xmlStrcmp(raidNode->name, (const xmlChar*)"raid") == 0) {
				...
				if (readXMLString(raidNode, "file", strValue)) {
					...
					ss << "raids/" << name << ".xml";
					file = ss.str();
					std::cout << "[Warning] Raids: file tag missing for raid "
				...
				if (readXMLInteger(raidNode, "interval2", intValue)) {
					...
					std::cout
					<< "[Error] Raids: interval2 tag missing for raid " << name
					<< std::endl;
					raidNode = raidNode->next;
					continue;
				}
				...
				if (readXMLInteger(raidNode, "margin", intValue)) {
					...
					std::cout << "[Warning] Raids: margin tag missing for raid "
					          << name << std::endl;
					margin = 0;
				}
				...
				bool ret = newRaid->loadFromXml(
				g_config.getString(ConfigManager::DATA_DIRECTORY) + "raids/" + file);
				if (!ret) {
...
void Raids::startup()
{
	...
	checkRaidsEvent = Scheduler::getScheduler().addEvent(
	createSchedulerTask(CHECK_RAIDS_INTERVAL * 1000, boost::bind(&Raids::checkRaids, this)));
	...
	if (!getRunning()) {
		...
		for (RaidList::iterator it = raidList.begin(); it != raidList.end(); ++it) {
			if (now >= (getLastRaidEnd() + (*it)->getMargin())) {
				if (MAX_RAND_RANGE * CHECK_RAIDS_INTERVAL / (*it)->getInterval() >=
				    (uint32_t)random_range(0, MAX_RAND_RANGE)) {
#ifdef __DEBUG_RAID__
					char buffer[32];
					time_t tmp = time(NULL);
					formatDate(tmp, buffer);
					std::cout << buffer << " [Notice] Raids: Starting raid "
					          << (*it)->getName() << std::endl;
#endif
					setRunning(*it);
					(*it)->startRaid();
	...
	if (doc) {
		...
		while (eventNode) {
			...
			if (event->configureRaidEvent(eventNode)) {
				...
				std::cout << "Raids: Error in file(" << _filename
...
void Raid::startRaid()
{
	...
	if (raidEvent) {
		...
		nextEventEvent = Scheduler::getScheduler().addEvent(
		createSchedulerTask(raidEvent->getDelay(), boost::bind(&Raid::executeRaidEvent, this, raidEvent)));
	}
}
...
void Raid::executeRaidEvent(RaidEvent* raidEvent)
{
	if (raidEvent->executeEvent()) {
		...
		if (newRaidEvent) {
			uint32_t ticks =
			(uint32_t)std::max(((uint32_t)RAID_MINTICKS),
			                   ((int32_t)newRaidEvent->getDelay() - raidEvent->getDelay()));
			nextEventEvent = Scheduler::getScheduler().addEvent(
			createSchedulerTask(ticks, boost::bind(&Raid::executeRaidEvent, this, newRaidEvent)));
		} else {
			resetRaid();
		}
	} else {
...
bool AnnounceEvent::configureRaidEvent(xmlNodePtr eventNode)
{
	...
	if (readXMLString(eventNode, "type", strValue)) {
		if (asLowerCaseString(strValue) == "warning") {
			...
			m_messageType = MSG_EVENT_ADVANCE;
			std::cout << "[Notice] Raid: Unknown type tag missing for announce event. "
			             "Using default: "
		...
		if (xmlStrcmp(monsterNode->name, (const xmlChar*)"monster") == 0) {
			...
			if (maxAmount == 0 && minAmount == 0) {
				if (readXMLInteger(monsterNode, "amount", intValue)) {
					...
					std::cout
					<< "[Error] Raid: amount tag missing for monster node."
					<< std::endl;
					return false;
				}
			}
...
bool ScriptEvent::configureRaidEvent(xmlNodePtr eventNode)
{
	...
	if (readXMLString(eventNode, "script", str)) {
		...
		std::cout
		<< "Error: [ScriptEvent::configureRaidEvent] No script file found for raid" << std::endl;
		return false;
	}
	return true;
}
