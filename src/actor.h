...
public:
	...
	{
		return shouldReload_;
	}
	void shouldReload(bool b)
	{
		shouldReload_ = b;
	}
	bool alwaysThink() const
	{
		return alwaysThink_;
	}
	void alwaysThink(bool b);
	bool onlyThink() const
	{
		return onlyThink_;
	}
	void onlyThink(bool b)
	{
		onlyThink_ = b;
	}
	...
	{
		return cType.defense();
	}
	virtual bool isPushable() const
	{
		return cType.pushable() && (baseSpeed > 0);
	}
	virtual bool isAttackable() const
	{
		return cType.isAttackable();
	}

	bool canPushItems() const
	{
		return cType.canPushItems();
	}
	bool canPushCreatures() const
	{
		return cType.canPushCreatures();
	}
	bool isHostile() const
	{
		return cType.isHostile();
	}
	virtual bool canSeeInvisibility() const
	{
		return Creature::isImmune(MECHANIC_INVISIBLE);
	}
	uint32_t getManaCost() const
	{
		return cType.manaCost();
	}
	...
	virtual void
	drainHealth(CombatType combatType, const CombatSource &combatSource, int32_t damage, bool showeffect);
	...
	const CreatureList &getFriendList() const
	{
		return friendList;
	}
	...
	{
		return getHealth() <= cType.fleeHealth();
	}

	bool isImmune(CombatType type) const;
	BlockType blockHit(CombatType combatType,
...
private:
	...
	bool canUseSpell(const Position &pos, const Position &targetPos, const SpellBlock &sb, uint32_t interval, bool &inRange);
