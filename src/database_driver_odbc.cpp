...
DatabaseODBC::DatabaseODBC()
{
	...
	if (!RETURN_SUCCESS(ret)) {
		std::cout << "SQLSetEnvAttr(SQL_ATTR_ODBC_VERSION): Failed to switch to "
		             "ODBC 3 version."
		          << std::endl;
	...
	if (!RETURN_SUCCESS(ret)) {
		std::cout << "SQLSetConnectAttr(SQL_ATTR_CONNECTION_TIMEOUT): Failed to "
	...
	if (!RETURN_SUCCESS(ret)) {
		std::cout << "Failed to connect to ODBC via DSN: " << dns << " (user " << user
		          << ")" << std::endl;
		SQLFreeHandle(SQL_HANDLE_DBC, m_handle);
		m_handle = NULL;
		return;
