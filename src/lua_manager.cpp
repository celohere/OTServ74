...
{
	if (manager) environment = manager->environment;
}
...
{
	if (lua_isnumber(state, index) != 0) return true;
...
{
	double d = popFloat();
	if (d < 0) throw Script::Error("Expected unsigned number, got negative number.");
	return uint32_t(d);
}
...
Tile *LuaState::popTile(Script::ErrorMode mode /* = Script::ERROR_THROW */)
{
	...
	if (!isTable(-1)) {
		HandleError(mode,
		            std::string("Couldn't pop tile, top object is not of valid type (") +
...
Town *LuaState::popTown(Script::ErrorMode mode /* = Script::ERROR_THROW */)
{
	...
	if (!isUserdata(-1)) {
		HandleError(mode,
		            std::string("Couldn't pop town, top object is not of valid type (") +
		            luaL_typename(state, -1) + ")");
		pop();
		return NULL;
	}
...
ChatChannel *LuaState::popChannel(Script::ErrorMode mode /* = Script::ERROR_THROW */)
{
	...
	if (!isUserdata(-1)) {
		HandleError(mode,
		            std::string("Couldn't pop channel, top object is not of valid type (") +
		            luaL_typename(state, -1) + ")");
		pop();
		return NULL;
	}
...
House *LuaState::popHouse(Script::ErrorMode mode /* = Script::ERROR_THROW */)
{
	...
	if (!isUserdata(-1)) {
		HandleError(mode,
		            std::string("Couldn't pop house, top object is not of valid type (") +
		            luaL_typename(state, -1) + ")");
		pop();
		return NULL;
	}
...
Waypoint_ptr LuaState::popWaypoint(Script::ErrorMode mode /* = Script::ERROR_THROW */)
{
	...
	if (!isUserdata(-1)) {
		HandleError(mode, std::string(
		                  "Couldn't pop waypoint, top object is not of valid type (") +
		                  luaL_typename(state, -1) + ")");
		pop();
		return Waypoint_ptr();
	}
...
Thing *LuaState::popThing(Script::ErrorMode mode /* = Script::ERROR_THROW */)
{
	...
	if (!isUserdata(-1)) {
		HandleError(mode,
		            std::string("Couldn't pop thing, top object is not of valid type (") +
		            luaL_typename(state, -1) + ")");
		pop();
		return NULL;
	}
...
Condition *LuaState::popCondition(Script::ErrorMode mode /* = Script::ERROR_THROW */)
{
	...
	if (!isUserdata(-1)) {
		HandleError(mode, std::string(
		                  "Couldn't pop condition, top object is not of valid type (") +
		                  luaL_typename(state, -1) + ")");
		pop();
		return NULL;
	}
...
Vocation *LuaState::popVocation(Script::ErrorMode mode /* = Script::ERROR_THROW */)
{
	...
	if (!isUserdata(-1)) {
		HandleError(mode, std::string(
		                  "Couldn't pop vocation, top object is not of valid type (") +
		                  luaL_typename(state, -1) + ")");
		pop();
		return NULL;
	}
...
void LuaStateManager::setupLuaStandardLibrary()
{
	...
	int32_t eret = luaL_loadfile(
	state, (g_config.getString(ConfigManager::DATA_DIRECTORY) + "scripts/error_module.lua").c_str());
	if (eret != 0) {
			...
			if (s == ".lua") {
				try {
					if (!loadFile(itr->path().string()))
						return false; // default construction yields
					// past-the-endath()))
...
: LuaState(manager), name(name), thread_state(0)
{
...
: LuaState(manager), name("Lua generated coroutine"), thread_state(0)
{
	state = L;
...
std::string LuaThread::report(const std::string &extramessage)
{
	...
	while (lua_getstack(state, level++, &ar) != 0) {
		...
		if (ar.currentline != -1) os << ar.currentline;

		os << "\t";

		int32_t tabcount = 20;
		if (!ar.name) ar.name = "<unknown>";
		os << ar.name;
