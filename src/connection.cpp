                                                   ...
                                                   ServicePort_ptr servicer)
{
	...
	boost::shared_ptr<Connection>(new Connection(socket, io_service, servicer));
...
: m_socket(socket), m_readTimer(io_service), m_writeTimer(io_service), m_io_service(io_service),
...
// dispatcher thread
	...
	if (m_connectionState != CONNECTION_STATE_REQUEST_CLOSE) {
		std::cout << "Error: [Connection::closeConnectionTask] m_connectionState = " << m_connectionState
		          << std::endl;
	...
	if (m_pendingWrite == 0 || m_writeError) {
		...
		// will be closed by onWriteOperation/handleWriteTimeout/handleReadTimeout
		// instead
	}
...
void Connection::releaseConnection()
{
	if (m_refCount > 0) {
		...
		createSchedulerTask(SCHEDULER_MINTICKS, boost::bind(&Connection::releaseConnection, this)));
	} else {
...
void Connection::internalSend(OutputMessage_ptr msg)
{
	...
	try {
		...
		boost::asio::async_write(getHandle(), boost::asio::buffer(msg->getOutputBuffer(),
		                                                          msg->getMessageLength()),
		                         boost::bind(&Connection::onWriteOperation, shared_from_this(),
