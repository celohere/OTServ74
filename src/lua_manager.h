...
class LuaState /*abstract*/
{
	...
	template <typename T> void setField(int32_t index, int32_t field_index, const T &t)
	{
		push(t);
	...
	{
		setField(LUA_GLOBALSINDEX, gname);
	}
	void setRegistryItem(const std::string &rname)
	{
		setField(LUA_REGISTRYINDEX, rname);
	}

	// Pushes value onto the stack
	void getGlobal(const std::string &gname)
	{
		getField(LUA_GLOBALSINDEX, gname);
	}
	void getRegistryItem(const std::string &rname)
	{
		getField(LUA_REGISTRYINDEX, rname);
	}
	...
	template <class E, int size_> void pushEnum(const Enum<E, size_> &);
	template <class E, int size_> void pushEnum(const BitEnum<E, size_> &);
	...
	void push(uint32_t ui)
	{
		pushUnsignedInteger(ui);
	}
	void push(int64_t i)
	{
		pushFloat((double)i);
	}
	void push(uint64_t ui)
	{
		pushFloat((double)ui);
	}
	void push(double d)
	...
	template <class E, int size_> void push(const Enum<E, size_> &e)
	{
		pushEnum<E, size_>(e);
	}
	template <class E, int size_> void push(const BitEnum<E, size_> &e)
	{
		pushEnum<E, size_>(e);
	}
...
// void push(int i) {pushInteger(i);}
	...
	// Don't use pushTable on a userdata class and vice-versa (events are table
	// classes, everything else userdata)
	...
	// If top object is table, and has __object member, pop table and push the
	// __object member
	void getMetaObject();
...
template <> inline bool LuaState::popValue<bool>()
{
	return popBoolean();
}
template <> inline uint32_t LuaState::popValue<uint32_t>()
{
	return popUnsignedInteger();
}
template <> inline int32_t LuaState::popValue<int32_t>()
{
	return (int32_t)popInteger();
}
template <> inline float LuaState::popValue<float>()
{
	return popFloat();
}
template <> inline double LuaState::popValue<double>()
{
	return popFloat();
}
template <> inline std::string LuaState::popValue<std::string>()
{
	return popString();
}
template <> inline uint64_t LuaState::popValue<uint64_t>()
{
	return (uint64_t)popFloat();
}
...
template <class E, int size_> inline void LuaState::pushEnum(const Enum<E, size_> &e)
{
	getGlobal(e.toString());
}

template <class E, int size_> inline void LuaState::pushEnum(const BitEnum<E, size_> &e)
{
	getGlobal(e.toString());
}
