...
bool Map::loadMap(const std::string &identifier, const std::string &type)
{
	...
	std::cout << ":: Loading map from: " << identifier << " " << loader->getSourceDescription()
	          << std::endl;
...
{
	if (z >= MAP_MAX_LAYERS) {
		std::cout << "ERROR: Attempt to set tile on invalid Z coordinate " << int(z) << "!"
		          << std::endl;
		return;
	}
...
bool Map::placeCreature(const Position &centerPos, Creature *creature, bool extendedPos /*=false*/, bool forceLogin /*=false*/)
{
	...
	for (uint32_t n = 1; n <= radius && !foundTile; ++n) {
		for (std::vector<relPair>::iterator it = relList.begin();
		     it != relList.end() && !foundTile; ++it) {
...
void Map::getSpectatorsInternal(SpectatorVec &list,
			...
			if (leafE) {
				...
				if (node_iter != node_end) {
					do {
						...
						if (cpos.y < (centerPos.y + minRangeY + offsetZ) ||
						    cpos.y > (centerPos.y + maxRangeY + offsetZ)) {
							continue;
						}
						if (cpos.x < (centerPos.x + minRangeX + offsetZ) ||
						    cpos.x > (centerPos.x + maxRangeX + offsetZ)) {
							continue;
						}
...
void Map::getSpectators(SpectatorVec &list,
                        const Position &centerPos,
                        ...
                        int32_t maxRangeY /*= 0*/)
{
	...
	if (minRangeX == 0 && maxRangeX == 0 && minRangeY == 0 && maxRangeY == 0 &&
	    multifloor == true && checkforduplicate == false) {
	...
	if (!foundCache) {
		...
		getSpectatorsInternal(list, centerPos, true, minRangeX, maxRangeX, minRangeY,
		                      maxRangeY, minRangeZ, maxRangeZ);
...
const SpectatorVec &Map::getSpectators(const Position &centerPos)
{
	...
	if (it != spectatorCache.end()) {
		...
		getSpectatorsInternal(list, centerPos, false, minRangeX, maxRangeX, minRangeY,
		                      maxRangeY, minRangeZ, maxRangeZ);
...
bool Map::canThrowObjectTo(const Position &fromPos,
...
bool Map::getPathTo(const Creature *creature, const Position &destPos, std::list<Direction> &listDir, int32_t maxSearchDist /*= -1*/)
{
		...
		if (n->x == endPos.x && n->y == endPos.y) {
			...
			for (int i = 0; i < 8; ++i) {
				...
				if (!outOfRange && (tile = canWalkTo(creature, pos))) {
					...
					// If it exists and the nodes already on them has a lower
					// cost (g) then we can ignore this neighbour node

					AStarNode *neighbourNode = nodes.getNodeInList(pos.x, pos.y);
					if (neighbourNode) {
						if (neighbourNode->g <= newg) {
							// The node on the closed/open list is
						...
						// Does not exist in the open/closed list, create a
						// new node
					...
					neighbourNode->h =
					nodes.getEstimatedDistance(neighbourNode->x, neighbourNode->y,
					                           endPos.x, endPos.y);
	...
	while (fpp.maxSearchDist != -1 || nodes.countClosedNodes() < 100) {
		...
		for (int32_t i = 0; i < dirCount; ++i) {
			...
			if (inRange && (tile = canWalkTo(creature, pos))) {
				...
				// Check if the node is already in the closed/open list
				// If it exists and the nodes already on them has a lower cost (g)
				// then we can ignore this neighbour node

				AStarNode *neighbourNode = nodes.getNodeInList(pos.x, pos.y);
				if (neighbourNode) {
					if (neighbourNode->f <= newf) {
						// The node on the closed/open list is cheaper than
						// this one
						continue;
					}
					...
					if (!neighbourNode) {
						if (found) {
							// not quite what we want, but we found
							// something
							break;
						}
...
int32_t AStarNodes::getMapWalkCost(const Creature *creature, AStarNode *node, const Tile *neighbourTile, const Position &neighbourPos)
{
	int cost = 0;
...
#else
			uint32_t index = ((swap_uint32(currentX) & 0x8000) >> 15) |
			                 ((swap_uint32(currentY) & 0x8000) >> 14);
#endif
