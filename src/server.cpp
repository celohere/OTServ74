...
ServiceManager::ServiceManager() : m_io_service(), death_timer(m_io_service), running(false)
{
}
...
void ServicePort::accept(Acceptor_ptr acceptor)
{
	try {
		...
		acceptor->async_accept(*socket, boost::bind(&ServicePort::onAccept, this, acceptor,
...
void ServicePort::onAccept(Acceptor_ptr acceptor,
                           ...
                           const boost::system::error_code &error)
{
	if (!error) {
		...
		if (remote_ip != 0 && g_bans.acceptConnection(remote_ip)) {
			...
			ConnectionManager::getInstance()->createConnection(socket, m_io_service,
			                                                   shared_from_this());
		...
		if (error != boost::asio::error::operation_aborted) {
			...
			if (!m_pendingStart) {
				...
				5000, boost::bind(&ServicePort::openAcceptor,
...
Protocol *ServicePort::make_protocol(bool checksummed, NetworkMessage &msg) const
{
	...
	for (std::vector<Service_ptr>::const_iterator it = m_services.begin(); it != m_services.end(); ++it) {
		...
		if (service->get_protocol_identifier() == protocolId &&
		    ((checksummed && service->is_checksummed()) || !service->is_checksummed())) {
			// Correct service! Create protocol and get on with it
			return service->make_protocol(Connection_ptr());
		}

		// We can ignore the other cases, they will most likely end up in return
		// NULL anyways.
	}
...
void ServicePort::open(IPAddressList ips, uint16_t port)
{
	...
	for (IPAddressList::iterator ip = ips.begin(); ip != ips.end(); ++ip) {
		try {
			// std::cout << "\n" << ip->to_string() << "\n";
			Acceptor_ptr aptr(new boost::asio::ip::tcp::acceptor(
			m_io_service, boost::asio::ip::tcp::endpoint(*ip, m_serverPort)));
			...
			g_scheduler.addEvent(createSchedulerTask(
			5000, boost::bind(&ServicePort::openAcceptor,
			                  boost::weak_ptr<ServicePort>(shared_from_this()), *ip, port)));
		}
	}
}
...
{
	for (std::vector<Acceptor_ptr>::iterator aptr = m_tcp_acceptors.begin();
	     aptr != m_tcp_acceptors.end(); ++aptr) {
		if ((*aptr)->is_open()) {
			boost::system::error_code error;
			(*aptr)->close(error);
			if (error) {
...
{
	for (std::vector<Service_ptr>::const_iterator svc_iter = m_services.begin();
	     svc_iter != m_services.end(); ++svc_iter) {
		Service_ptr svc = *svc_iter;
		if (svc->is_single_socket()) return false;
	}
