				...
				while (tmpNode) {
					if (xmlStrcmp(tmpNode->name, (const xmlChar *)"monster") == 0) {
						...
						if (interval >= MINSPAWN_INTERVAL) {
							...
							std::cout
							<< "[Warning] Spawns::loadFromXml " << name << " "
							<< pos << " spawntime can not be less than "
							<< MINSPAWN_INTERVAL / 1000 << " seconds."
							<< std::endl;
						}
					} else if (xmlStrcmp(tmpNode->name,
					                     (const xmlChar *)"npc") == 0) {
...
void Spawn::startSpawnCheck()
{
	if (checkSpawnEvent == 0) {
		checkSpawnEvent = Scheduler::getScheduler().addEvent(
...
bool Spawn::spawnMonster(uint32_t spawnId, MonsterType *mType, const Position &pos, Direction dir, bool startup /*= false*/)
{
	...
	if (startup) {
		// No need to send out events to the surrounding since there is no one out there to
		// listen!
