...
protected:
	...
	template <class ET> void registerEnum();
...
template <class ET> inline void Script::Manager::registerEnum()
{
	...
	// std::cout << "Global " << ET::name() << " table is " <<
	// lua_topointer(state, 1) << std::endl;
	// Expose members
	int n = 1;
	for (typename ET::iterator ei = ET::begin(); ei != ET::end(); ++ei, ++n) {
		...
		int in = 1;
		for (std::vector<std::string>::const_iterator str = stringValues.begin();
		     str != stringValues.end(); ++str, ++in)
			setField(3, in, *str);
		...
		// std::cout << "set field of " << lua_topointer(state, 1) << " to " <<
		// lua_topointer(state, -1) << " " << typeName(-1) << std::endl;
	...
	// Another added bonus is that it's impossible to create new Enum values of
	// this
