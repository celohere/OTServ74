...
bool Mailbox::sendItem(Item *item)
{
	...
	if (Player *player = g_game.getPlayerByName(receiver)) {
		...
		if (!IOPlayer::instance()->loadPlayer(player, receiver)) {
			...
			std::cout << "Failure: [Mailbox::sendItem], can not load player: " << reciever
			          << std::endl;
#endif
			delete player;
			return false;
		...
		if (g_game.getPlayerByName(playerName)) {
			std::cout << "Failure: [Mailbox::sendItem], receiver is online: " << receiver
			          << "," << playerName << std::endl;
			delete player;
			return false;
		}
...
bool Mailbox::getReceiver(Item *item, std::string &name, uint32_t &dp)
{
	...
	if (item->getID() ==
		...
		std::cout
		<< "Mailbox::getReciver error, trying to get reciecer from unkown item! ID:: "
		<< item->getID() << "." << std::endl;
		return false;
	}
