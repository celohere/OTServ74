...
#if !defined(__WINDOWS__)
// Runfile, for running OT as daemon in the background. If the server is
// shutdown by internal
// means, we need to clear the file to notify the daemon manager of our change
// in status.
...
#if defined __DEBUG__MOVESYS__ || defined __DEBUG_HOUSES__ || defined __DEBUG_LUASCRIPTS__ || \
defined __DEBUG_RAID__ || defined __DEBUG_NET_DETAIL__ || defined __DEBUG_SQL__
	...
	if (getuid() == 0 || geteuid() == 0) {
		std::cout << std::endl
		          << "OTServ executed as root user, please login with a normal user." << std::endl;
		return 1;
	}
#endif
	...
	while (argi != args.end()) {
		...
		if (arg == "-p" || arg == "--port") {
			...
			<< "Usage: otserv {-i|-p|-c|-r|-l}\n"
			   ...
			   "\n"
			   "\t\t\t\tglobal IP.\n"
			   "\t-p, --port $1 $2\t\tPort ($2) for server to listen on $1 is "
			   "type\n"
			   "\t\t\t\t(status, login, admin).\n"
			   "\t-c, --config $1\t\tAlternate config file path.\n"
			   "\t-l, --log-file $1 $2\tAll standard output will be logged to "
			   "the $1\n"
...
void badAllocationHandler()
{
	...
	puts("Allocation failed, server out of memory.\nDecrese the size of your map "
	     "or compile in 64-bit mode.");
	char buf[1024];
...
void mainLoader(const CommandLineOptions &command_opts, ServiceManager *service_manager)
{
	...
	if (GetLastError() == ERROR_ALREADY_EXISTS)
		ErrorMessage("There's an another instance of the OTServ running with the "
		             "same login port, please shut it down first or change ports "
		             "for this one."
		             "\nIf you want to run multiple servers, disable login ports "
		             "for all but one of them.");
#endif

#if defined(PKGDATADIR) && !defined(__WINDOWS__) // I dont care enough to port
	// this to win32, prolly not
	// needed
	// let's fix the datadir, if necessary...
	if (access(g_config.getString(ConfigManager::DATA_DIRECTORY).c_str(),
	           F_OK)) { // check if datadir exists
		// if not then try replacing it with "global" datadir
		std::cout << ":: No datadir '" << g_config.getString(ConfigManager::DATA_DIRECTORY).c_str()
		          << "', using a system-wide one" << std::endl;
	...
	std::cout << ":: Using data directory "
	          << g_config.getString(ConfigManager::DATA_DIRECTORY).c_str() << "... " << std::flush;
	/* Won't compile! access is not standard
	if (access(g_config.getString(ConfigManager::DATA_DIRECTORY).c_str(), F_OK)) {
	...
	*/
	...
	std::cout << ":: Checking Connection to Database "
	...
	DBResult* result;
	if(!(result = db->storeQuery("SELECT `value` FROM `schema_info` WHERE `name` =
	'version';"))){
	...
	if(schema_version != CURRENT_SCHEMA_VERSION){
	  ErrorMessage("Your database is outdated. Run the dbupdate utility to update
	...
	*/
	...
	if (g_config.getString(ConfigManager::PASSWORD_SALT) != "") std::cout << " [salted]";
	std::cout << std::endl;
	if (!g_game.loadMap(g_config.getString(ConfigManager::MAP_FILE))) {
		...
		filename.str("");
		filename << g_config.getString(ConfigManager::DATA_DIRECTORY)
	...
	if (!(world_result =
	      db->storeQuery("SELECT `name`, `port` FROM `worlds` WHERE `id` = '" + world_id + "'"))) {
	...
	// Load IP Address(es)
	// Primary IP is the "best" IP we can find (first global, then net adapter,
	// then localhost)
	...
	if (g_config.getString(ConfigManager::IP) == "auto") {
		...
		for (server = servers.begin(); server != servers.end(); ++server) {
			...
			if (200 == request.url(*server).method(HTTP::GET).fetch().responseCode()) {
				std::cout << " [done]" << std::endl;
	...
	if (gethostname(szHostName, 128) == 0) {
		...
		if (he) {
			if (useLocal) std::cout << ":: LAN IP address(es):  ";
			...
			while (addr[0] != NULL) {
				...
				if (ipList.back() == globalIP) ownsGlobal = true;
	...
	ipQuery << "UPDATE `worlds` SET `ip` = " << swap_uint32(globalIP.to_v4().to_ulong())
	        << " WHERE `id` = " << world_id;
	...
	// Legacy protocols (they need to listen on login port as all old server only
	// used one port
