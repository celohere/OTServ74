...
defined __DEBUG_LUASCRIPTS__ || defined __DEBUG_RAID__ || defined __DEBUG_NET__ || defined __DEBUG_SQL__
...
!defined(__WINDOWS__) // i dont care enough to port this to win32, prolly not needed
	...
	if (access(g_config.getString(ConfigManager::DATA_DIRECTORY).c_str(),
		...
		if (access(g_config.getString(ConfigManager::DATA_DIRECTORY).c_str(),
		           F_OK)) { // check if this new one exists
