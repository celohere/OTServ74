...
public:
	...
	*
	* Retruns instance of database handler. Don't create database (or drivers) instances in your
	* code - instead of it use Database::instance(). This method stores static instance of
	* connection class internaly to make sure exacly one instance of connection is created for
	* entire system.
	*
...
protected:
	...
	* @note
	*	If your database system doesn't support transactions you should return true - it's
	*not feature test, code should work without transaction, just will lack integrity.
	*/
...
public:
	...
	*/
	 ...
	 *
	 * @return id of last inserted row, 0 if last query did not result in any rows with
	 * auto_increment keys
	 */
	...
	*/
	 ...
	 *
	 * @return id on success, 0 if last query did not result on any rows with auto_increment
	 * keys
	 */
...
public:
	...
	*\return a PropStream that is initiated with the blob data field, if not exist it returns
	*NULL.
	*\param s The name of the field
	*/
