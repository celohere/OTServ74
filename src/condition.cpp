...
Condition *Condition::createCondition(ConditionId id, uint32_t ticks, uint32_t sourceId /*= 0*/, uint32_t flags /*= 0*/)
{
	...
	case enums::CONDITION_POISONED:
		return Condition::createCondition(id.toString(), ticks, MECHANIC_NONE,
		                                  COMBAT_EARTHDAMAGE, sourceId);
	case enums::CONDITION_ELECTRIFIED:
		return Condition::createCondition(id.toString(), ticks, MECHANIC_NONE,
		                                  COMBAT_ENERGYDAMAGE, sourceId);
	...
	case enums::CONDITION_HUNTING: {
		Condition *condition =
		Condition::createCondition(id.toString(), ticks, MECHANIC_NONE, COMBAT_NONE, sourceId);
		if (condition) {
			ConditionEffect effect =
			ConditionEffect::createModStamina(1000, -g_config.getNumber(ConfigManager::RATE_STAMINA_LOSS));
	...
	case enums::CONDITION_INFIGHT:
		return Condition::createCondition(id.toString(), ticks, MECHANIC_NONE, COMBAT_NONE,
		                                  sourceId, enums::ICON_SWORDS);
	case enums::CONDITION_HASTE:
		return Condition::createCondition(id.toString(), ticks, MECHANIC_NONE, COMBAT_NONE,
		                                  sourceId, enums::ICON_HASTE);
	case enums::CONDITION_PARALYZED:
		return Condition::createCondition(id.toString(), ticks, MECHANIC_PARALYZED,
		                                  COMBAT_NONE, sourceId, enums::ICON_PARALYZE);
	case enums::CONDITION_DRUNK:
		return Condition::createCondition(id.toString(), ticks, MECHANIC_DRUNK, COMBAT_NONE,
		                                  sourceId, enums::ICON_DRUNK);
	case enums::CONDITION_MANASHIELD:
		return Condition::createCondition(id.toString(), ticks, MECHANIC_NONE, COMBAT_NONE,
		                                  sourceId, enums::ICON_MANASHIELD);
...
Condition *Condition::createCondition(const std::string &name,
                                      uint32_t ticks,
                                      MechanicType mechanicType /*= MECHANIC_NONE*/,
                                      CombatType combatType /*= MECHANIC_NONE*/,
                                      uint32_t sourceId /*= 0*/,
                                      uint32_t flags /*= 0*/)
{
...
Condition::Condition(const Condition &rhs)
{
	...
	for (std::list<ConditionEffect>::const_iterator it = rhs.effectList.begin();
	     it != rhs.effectList.end(); ++it) {
		addEffect(*it);
	}
}
...
bool Condition::onUpdate(Creature *creature, const Condition *addCondition)
{
	...
	if (!fullUpdate) {
		...
		for (std::list<ConditionEffect>::const_iterator it = addCondition->effectList.begin();
		     it != addCondition->effectList.end(); ++it) {
	...
	if (fullUpdate) {
		// Condition has been changed, maybe from a script reload, doing a full
		// update
		for (std::list<ConditionEffect>::iterator it = effectList.begin();
		     it != effectList.end(); ++it) {
	...
	if (percent != 0) {
		...
		if (Player *player = creature->getPlayer()) {
			...
			case enums::STAT_SOULPOINTS:
				return (int32_t)((float)player->getPlayerInfo(PLAYERINFO_SOUL) *
				                 (percent / 100.f));
...
bool ConditionEffect::onBegin(Creature *creature)
{
	...
	case ConditionEffect::PERIODIC_DAMAGE: {
		ConditionEffect::ModPeriodicDamage &modPeriodicDamage =
		getModEffect<ConditionEffect::ModPeriodicDamage>();

		if (modPeriodicDamage.rounds == 0) {
			// average damage
			modPeriodicDamage.total =
			random_range(modPeriodicDamage.min, modPeriodicDamage.max);
	...
	case ConditionEffect::MOD_SPEED: {
		...
		int32_t delta = (int32_t)std::ceil(
		((float)creature->getBaseSpeed()) * ((float)modSpeed.percent) / 100 + modSpeed.value);
	...
	case ConditionEffect::SHAPESHIFT: {
		...
		getModEffect<const ConditionEffect::ModShapeShift>();
		OutfitType outfit;
	...
	case ConditionEffect::DISPEL: {
		const ConditionEffect::ModDispel &modDispel =
		getModEffect<const ConditionEffect::ModDispel>();
	...
	if (Player *player = creature->getPlayer()) {
		...
		case ConditionEffect::MOD_STAT: {
			...
			int32_t delta =
			getStatValue(creature, modStat.type, modStat.percent, modStat.value);
		...
		case ConditionEffect::MOD_SKILL: {
			...
			int32_t delta =
			getSkillValue(creature, modSkill.type, modSkill.percent, modSkill.value);
...
bool ConditionEffect::onEnd(Creature *creature, ConditionEnd reason)
{
	...
	if (Player *player = creature->getPlayer()) {
		...
		case ConditionEffect::MOD_STAT: {
			// revert our changes
			const ConditionEffect::ModStat &modStat =
			getModEffect<const ConditionEffect::ModStat>();
		...
		case ConditionEffect::MOD_SKILL: {
			// revert our changes
			const ConditionEffect::ModSkill &modSkill =
			getModEffect<const ConditionEffect::ModSkill>();
	...
	if (tickCount >= interval) {
		...
		case ConditionEffect::PERIODIC_DAMAGE: {
			...
			if (type == ConditionEffect::PERIODIC_HEAL) {
				...
				// std::cout << "Healing " << heal << " to " << creature->getName()
				// <<
				// std::endl;
				if (!g_game.combatBlockHit(COMBAT_HEALING, combatSource, creature,
				                           heal, false, false)) {
				...
				// modPeriodicDamage.type.toString() << " to " <<
				// creature->getName() <<
				// std::endl;
				if (!g_game.combatBlockHit(modPeriodicDamage.type, combatSource,
				                           creature, damage, false, false)) {
					g_game.combatDamage(modPeriodicDamage.type, combatSource,
					                    creature, damage);
				}
			}
			...
			if (const MagicField *field = creature->getParentTile()->getFieldItem()) {
				if (field->getCombatType() == modPeriodicDamage.type) {
					// The creature is still standing in the field so the damage
					// should
			...
			if (modPeriodicDamage.total != 0) {
				...
				int32_t curRounds = (int32_t)std::ceil(
				((float)modPeriodicDamage.percent) / modPeriodicDamage.first);
				...
				if (modPeriodicDamage.sum >= modPeriodicDamage.total ||
				    modPeriodicDamage.first == 0) {
					return false;
				}
			} else {
				// number of rounds
		...
		case ConditionEffect::REGEN_SOUL: {
			if (creature->getZone() != ZONE_PROTECTION) {
				ConditionEffect::ModRegen modRegen =
				getModEffect<ConditionEffect::ModRegen>();
				int32_t value =
				getStatValue(creature, modRegen.type, modRegen.percent, modRegen.value);
				// std::cout << "Regen " << value << " to " << creature->getName()
				// <<
				// std::endl;
		...
		case ConditionEffect::PERIODIC_MOD_STAMINA: {
			if (Player *player = creature->getPlayer()) {
				ConditionEffect::ModPeriodicStamina modStamina =
				getModEffect<ConditionEffect::ModPeriodicStamina>();
