...
bool ConditionDamage::unserializeProp(ConditionAttr_t attr, PropStream &propStream)
{
	if (attr == CONDITIONATTR_DELAYED) {
		...
		if (!propStream.GET_INT32(damageInfo.timeLeft) || !propStream.GET_INT32(damageInfo.value) ||
	...
	if (damageList.empty()) {
		...
		if (amount != 0) {
			if (startDamage > maxDamage) {
				...
				startDamage =
				std::max((int32_t)1, (int32_t)std::ceil(((float)amount / 20.0)));
			}
...
bool ConditionOutfit::unserializeProp(ConditionAttr_t attr, PropStream &propStream)
{
	if (attr == CONDITIONATTR_OUTFIT) {
		Outfit_t outfit;
		if (!propStream.GET_UINT32(outfit.lookType) || !propStream.GET_UINT32(outfit.lookTypeEx) ||
		    !propStream.GET_UINT32(outfit.lookHead) || !propStream.GET_UINT32(outfit.lookBody) ||
		    !propStream.GET_UINT32(outfit.lookLegs) || !propStream.GET_UINT32(outfit.lookFeet)) {
			return false;
		}
