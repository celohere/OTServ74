...
bool WaitingList::clientLogin(const Player *player)
{
	...
	if (waitList.empty() &&
	    Status::instance()->getPlayersOnline() < Status::instance()->getMaxPlayersOnline()) {
...
#ifdef __DEBUG__WATINGLIST__
	std::cout << "Name: " << player->getName() << "(" << waitList.size() + 1 << ")"
	          << " has been added to the waiting list" << std::endl;
#endif
...
#ifdef __DEBUG__WATINGLIST__
	std::cout << "WaitingList::getSlot error, trying to find slot for unknown acc: "
	          << player->getAccountId() << " with ip " << player->getIP() << std::endl;
#endif
