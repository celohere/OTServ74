...
enum WorldType_t { WORLD_TYPE_NO_PVP = 1, WORLD_TYPE_PVP = 2, WORLD_TYPE_PVP_ENFORCED = 3 };
...
public:
	...
	uint32_t getInFightTicks()
	{
		return inFightTicks;
	}
	int32_t getExhaustionTicks()
	{
		return exhaustionTicks;
	}
	int32_t getFightExhaustionTicks()
	{
		return fightExhaustionTicks;
	}
	int32_t getHealExhaustionTicks()
	{
	...
	ReturnValue
	internalMoveCreature(Creature *creature, Cylinder *fromCylinder, Cylinder *toCylinder, uint32_t flags = 0);

	ReturnValue internalMoveItem(Cylinder *fromCylinder,
	                             Cylinder *toCylinder,
	                             int32_t index,
	                             Item *item,
	                             uint32_t count,
	...
	ReturnValue internalAddItem(Cylinder *toCylinder,
	                            Item *item,
	...
	/**
	  ...
	  * \param subType is the extra type an item can have such as charges/fluidtype, default is
	 * -1
	...
	bool playerUseItemEx(uint32_t playerId,
	                     ...
	                     uint8_t toStackPos,
	                     uint16_t toSpriteId);
	bool playerUseItem(uint32_t playerId, const Position &pos, uint8_t stackPos, uint8_t index, uint16_t spriteId);
	...
	bool playerSay(uint32_t playerId,
	               uint16_t channelId,
	               SpeakClasses type,
	...
	bool combatBlockHit(CombatType_t combatType,
	                    Creature *attacker,
	                    Creature *target,
	...
	bool combatChangeHealth(CombatType_t combatType,
