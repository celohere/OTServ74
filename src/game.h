...
enum LightState_t { LIGHT_STATE_DAY, LIGHT_STATE_NIGHT, LIGHT_STATE_SUNSET, LIGHT_STATE_SUNRISE };
...
class Game
{
	...
	/**
	  ...
	  * \return Int 0 built-in spawns, 1 needs xml spawns, 2 needs sql spawns, -1
	 * if got error
	  */
	...
	*/
	...
	uint32_t getInFightTicks() const
	{
		return inFightTicks;
	}
	uint32_t getExhaustionTicks() const
	{
		return exhaustionTicks;
	}
	uint32_t getAddExhaustionTicks() const
	{
		return addExhaustionTicks;
	}
	uint32_t getFightExhaustionTicks() const
	{
		return fightExhaustionTicks;
	}
	uint32_t getHealExhaustionTicks() const
	{
		return healExhaustionTicks;
	}
	uint32_t getStairhopExhaustion() const
	{
		return stairhopExhaustion;
	}

	Cylinder *internalGetCylinder(Player *player, const Position &pos);
	Thing *internalGetThing(Player *player,
	                        const Position &pos,
	                        int32_t index,
	                        uint32_t spriteId = 0,
	...
	/**
	  ...
	  * it is up to the caller of the function to delete the pointer - if the
	 * player is offline
	...
	/**
	  * Returns a list of players based on a string name identifier, with support
	 * for the "~" wildcard.
	  * \param s is the name identifier, with or without wildcard
	...
	/** Place Creature on the map without sending out events to the surrounding.
	  ...
	  * \param forced If true, placing the creature will not fail because of
	 * obstacles (creatures/items)
	  */
	...
	void getSpectators(SpectatorVec &list,
	                   ...
	                   int32_t maxRangeY = 0)
	{
		map->getSpectators(list, centerPos, checkforduplicate, multifloor, minRangeX,
	...
	ReturnValue
	internalMoveCreature(Creature *actor, Creature *creature, Direction direction, uint32_t flags = 0);
	ReturnValue internalMoveCreature(Creature *actor,
	                                 Creature *creature,
	                                 Cylinder *fromCylinder,
	                                 Cylinder *toCylinder,
	                                 uint32_t flags = 0);

	ReturnValue internalMoveItem(Creature *actor,
	                             Cylinder *fromCylinder,
	                             Cylinder *toCylinder,
	                             int32_t index,
	                             Item *item,
	...
	ReturnValue internalAddItem(Creature *actor,
	                            Cylinder *toCylinder,
	                            Item *item,
	...
	ReturnValue
	internalRemoveItem(Creature *actor, Item *item, int32_t count = -1, bool test = false, uint32_t flags = 0);
	 ...
	 * charges/fluidtype, default is -1
	...
	bool playerMoveThing(uint32_t playerId,
	                     const Position &fromPos,
	                     uint16_t spriteId,
	                     uint8_t fromStackPos,
	                     const Position &toPos,
	                     uint8_t count);
	bool playerMoveCreature(uint32_t playerId,
	                        uint32_t movingCreatureId,
	                        const Position &movingCreatureOrigPos,
	                        const Position &toPos);
	bool playerMoveItem(uint32_t playerId,
	                    const Position &fromPos,
	                    uint16_t spriteId,
	                    uint8_t fromStackPos,
	                    const Position &toPos,
	...
	bool playerUseItem(uint32_t playerId, Position pos, uint8_t stackPos, uint8_t index, uint16_t spriteId, bool isHotkey);
	bool playerUseItemEx(uint32_t playerId,
	                     Position fromPos,
	                     uint8_t fromStackPos,
	                     ...
	                     uint16_t toSpriteId,
	                     bool isHotkey);
	bool playerUseBattleWindow(uint32_t playerId,
	                           Position fromPos,
	                           uint8_t fromStackPos,
	                           uint32_t creatureId,
	                           uint16_t spriteId,
	...
	bool playerShopPurchase(uint32_t playerId,
	                        uint16_t spriteId,
	                        uint8_t count,
	                        uint8_t amount,
	                        bool ignoreCapacity = false,
	                        bool buyWithBackpack = false);
	bool playerShopSell(uint32_t playerId, uint16_t spriteId, uint8_t count, uint8_t amount);
	...
	bool playerSay(uint32_t playerId, uint16_t channelId, SpeakClass type, std::string receiver, std::string text);
	...
	bool playerViolationWindow(uint32_t playerId,
	...
	bool playerReportViolation(uint32_t playerId,
	...
	// Script event callbacks, all are in the game class so we don't have to
	// include the script files
	bool onAccountLogin(std::string &name,
	                    uint32_t &number,
	                    std::string &password,
	                    time_t &premiumEnd,
	                    uint32_t &warnings,
	...
	bool getPathToEx(const Creature *creature,
	...
	bool combatBlockHit(CombatType combatType,
	...
	bool combatDamage(CombatType combatType, Creature *attacker, Creature *target, int32_t amount, bool showEffect = true);
	bool combatDamage(CombatType combatType,
	                  CombatSource combatSource,
	                  Creature *target,
	                  int32_t amount,
