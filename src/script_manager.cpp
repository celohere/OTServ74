...
Manager::Manager(Script::Environment &e)
: LuaStateManager(NULL), function_id_counter(1), event_nested_level(0)
{
	manager = this;
	environment = &e;
...
int Manager::luaGetClassInstanceID(lua_State *L)
{
	...
	*/
	...
	// Check that's it's a userdata instance (and not a table class, even if
	// that's impossible)
...
{
	// Do NOT allocate complex types here, lua_error is called, which causes a
	// longjump,
	...
	// We must allocate manually, since lua_error is called no destructors will
	// work
...

	{
		...
		try {
			...
			for (std::vector<ComposedTypeDeclaration>::const_iterator ctditer =
			     cc->parameters.begin();
			     ctditer != cc->parameters.end(); ++ctditer) {
				if (ctditer->optional_level == 0) required_arguments += 1;
			}
			...
			// parsed_argument_count is how many arguments we have parsed in the loop
			// so far
			...
			for (std::vector<ComposedTypeDeclaration>::const_iterator ctditer =
				...
				if ((size_t)argument_count < cc->parameters.size() &&
				    required_arguments - parsed_argument_count > 0 &&
				    !ctd.default_value.empty() && ctd.optional_level > 0) {
					// We got an optional argument, and one is missing on this
					// spot!
					...
					if (ctd.default_value.type() == typeid(std::string))
						...
						throw Error("Cannot deduce type of optional "
			...
			// Cleanup
			if (private_thread) private_thread->~LuaThread();
			return ret;
		} catch (Script::Error &err) {
			// We can't use lua_error in the C++ function as it doesn't call
	...
	if (s[0] == ')') {
		...
		while (true) {
			...
			if (s[0] == ',') {
				...
				while (true) {
					if (s[0] == ',') {
						// Declaration continues after this, but we
						// decreased optional_level
						// already
						s.erase(s.begin());
						parseWhitespace(s);
						...
						if (optional_level == 0) break;

						// If declaration does not continue, after a ']'
						// only another ']'
						// can follow
						assert(s[0] == ']');
						s.erase(s.begin());
...
{
	std::string::iterator iter = s.begin();
	assert((*iter >= 'a' && *iter <= 'z') || (*iter >= 'A' && *iter <= 'Z') || (*iter == '_'));
	++iter;
	while ((*iter >= 'a' && *iter <= 'z') || (*iter >= 'A' && *iter <= 'Z') || (*iter == '_') ||
	       (*iter >= '0' && *iter <= '9')) {
		++iter;
	}
...
Manager::ComposedTypeDeclaration Manager::parseTypeDeclaration(std::string &s)
{
	...
	while (true) {
		...
		assert(type == "mixed" || type == "boolean" || type == "number" ||
		       type == "string" || type == "function" || type == "userdata" ||
		       type == "thread" || type == "table" || class_list.find(type) != class_list.end());
...
void Manager::registerClass(const std::string &cname, const std::string &parent_class)
{
	...
	lua_setfield(state, -2, "__index"); // Set the index metamethod for the
	// redirect table to the base class table
...
void Manager::registerMemberFunction(const std::string &cname, const std::string &fdecl, CallbackFunctionType cfunc)
{
	...
	// Create a unique ID for the function, and associate it with the composed
	// callback
	uint32_t function_id = ++function_id_counter;
	function_map[function_id] = func;
