...
void Monster::onCreatureMove(const Creature *creature,
                             const Tile *newTile,
...
void Monster::onCreatureLeave(Creature *creature)
{
	...
	if (isFriend(creature)) {
		...
		if (it != friendList.end()) {
			...
			std::cout << "Monster: " << creature->getName()
...
BlockType_t Monster::blockHit(Creature *attacker,
...
{
	// the walkCache need to be updated if the monster becomes "resistent" to the damage, see
	// Tile::__queryAdd()
...
{
	// the walkCache need to be updated if the monster loose the "resistent" to the damage, see
	// Tile::__queryAdd()
	if (type == CONDITION_FIRE || type == CONDITION_ENERGY || type == CONDITION_POISON) {
		updateMapCache();
	}
		...
		if (isSummon()) {
			if (!attackedCreature) {
				if (getMaster() && getMaster()->getAttackedCreature()) {
					...
					// Our master has not ordered us to attack anything, lets
					// follow him around instead.
...
void Monster::onThinkYell(uint32_t interval)
{
	if (mType->yellSpeedTicks > 0) {
		...
		if (yellTicks >= mType->yellSpeedTicks) {
			...
			if (!mType->voiceVector.empty() &&
			    (mType->yellChance >= (uint32_t)random_range(1, 100))) {
...
void Monster::pushItems(Tile *tile)
{
	...
	for (int32_t i = downItemSize - 1; i >= 0; --i) {
		...
		if (item && item->hasProperty(MOVEABLE) &&
...
Item *Monster::getCorpse()
{
	...
	if (corpse) {
		...
		if (getKillers(&lastHitCreature, &mostDamageCreature) && mostDamageCreature) {
			...
			} else if (mostDamageCreature->getMaster() &&
			           mostDamageCreature->getMaster()->getPlayer()) {
