...
ItemAttributes::ItemAttributes() : attributes(NULL)
{
}

ItemAttributes::ItemAttributes(const ItemAttributes &o)
{
	if (o.attributes) attributes = new AttributeMap(*o.attributes);
}
...
{
	if (!attributes) attributes = new AttributeMap;
}
...
{
	if (!attributes) return boost::any();

	AttributeMap::iterator iter = attributes->find(key);
	if (iter != attributes->end()) return iter->second.get();
	return boost::any();
}
...
{
	if (m_type == ItemAttribute::STRING) delete m_var.string;
}
...
{
	if (m_type == INTEGER) return &m_var.signedInteger;
...
{
	if (m_type == FLOAT) return &m_var.signedFloat;
...
{
	if (m_type == BOOLEAN) return &m_var.boolean;
...
bool ItemAttributes::unserializeAttributeMap(PropStream &stream)
{
	...
	if (stream.GET_USHORT(n)) {
		...
		while (n--) {
			if (!stream.GET_STRING(key)) return false;
			if (!attrib.unserialize(stream)) return false;
...
bool ItemAttribute::unserialize(PropStream &stream)
{
	...
	// do not call here set(...) or any other function depending on m_type which
	// may result in deallocating phantom string !

	// read contents
	switch (m_type) {
	case STRING: {
		m_var.string = new std::string;
		if (!stream.GET_LSTRING(*m_var.string)) return false;
	...
	case INTEGER:
	case FLOAT: {
		if (!stream.GET_ULONG(m_var.unsignedInteger)) return false;

		break;
	}

	case BOOLEAN: {
		if (!stream.GET_UCHAR(m_var.unsignedChar)) return false;
