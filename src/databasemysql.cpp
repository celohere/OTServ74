...
DatabaseMySQL::DatabaseMySQL()
{
	...
	if (MYSQL_VERSION_ID < 50019) {
		// mySQL servers < 5.0.19 has a bug where MYSQL_OPT_RECONNECT is (incorrectly) reset
		// by mysql_real_connect calls
		// See http://dev.mysql.com/doc/refman/5.0/en/mysql-options.html for more
		// information.
		mysql_options(&m_handle, MYSQL_OPT_RECONNECT, &reconnect);
		std::cout << "[Warning] Outdated mySQL server detected. Consider upgrading to a "
		             "newer version."
		          << std::endl;
	}
...
DBResult *DatabaseMySQL::storeQuery(const std::string &query)
{
	...
	// error occured
	if (!m_res) {
		std::cout << "mysql_store_result(): " << query
		          << ": MYSQL ERROR: " << mysql_error(&m_handle) << std::endl;
		int error = mysql_errno(&m_handle);
