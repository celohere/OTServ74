...
DatabaseSQLite::DatabaseSQLite()
{
	...
	if (fin.fail()) {
		std::cout << "Failed to initialize SQLite connection. File "
		          << g_config.getString(ConfigManager::SQL_DB) << " does not exist." << std::endl;
		return;
	}
	fin.close();
...
bool DatabaseSQLite::internalQuery(const std::string &query)
{
	...
	if (ret != SQLITE_OK && ret != SQLITE_DONE && ret != SQLITE_ROW) {
		sqlite3_finalize(stmt);
		std::cout << "sqlite3_step(): SQLITE ERROR: " << sqlite3_errmsg(m_handle) << " ("
		          << buf << ")" << std::endl;
		return false;
	}
...
{
	listNames_t::iterator it = m_listNames.find(s);
	if (it != m_listNames.end()) return (uint32_t)sqlite3_column_int64(m_handle, it->second);
