...
private:
	...
	void sendToChannel(const Creature* creature,
	                   SpeakClasses type,
	...
	void GetFloorDescription(NetworkMessage_ptr msg, int x, int y, int z, int width, int height, int offset, int& skip);

	// translate a map area to clientreadable format
	void GetMapDescription(unsigned short x,
	                       unsigned short y,
	                       unsigned char z,
	...
	void AddCreatureSpeak(NetworkMessage_ptr msg,
	...
	void MoveUpCreature(NetworkMessage_ptr msg,
	                    const Creature* creature,
	                    const Position& newPos,
	                    const Position& oldPos,
	                    uint32_t oldStackPos);
	void MoveDownCreature(NetworkMessage_ptr msg,
	                      const Creature* creature,
	                      const Position& newPos,
	                      const Position& oldPos,
	                      uint32_t oldStackPos);
	...
	template <class T1, class f1, class r> void addGameTask(r (Game::*f)(f1), T1 p1);
	...
	void
	addGameTask(r (Game::*f)(f1, f2, f3, f4, f5, f6, f7, f8), T1 p1, T2 p2, T3 p3, T4 p4, T5 p5, T6 p6, T7 p7, T8 p8);
