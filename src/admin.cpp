...
ProtocolAdmin::ProtocolAdmin(Connection_ptr connection) : Protocol(connection)
{
...
void ProtocolAdmin::onRecvFirstMessage(NetworkMessage &msg)
{
	...
	if (output) {
		...
		output->AddU32(
		g_adminConfig->getProtocolOptions()); // protocol options(encryption, ...)
	...
	if (output) {
		...
		case ENCRYPTION_NO_SET: {
			if (g_adminConfig->requireEncryption()) {
				...
				if (recvbyte != AP_MSG_ENCRYPTION && recvbyte != AP_MSG_KEY_EXCHANGE) {
					...
					getConnection()->closeConnection();
					addLogLine(this, LOGTYPE_WARNING, 1,
					           "wrong command while ENCRYPTION_NO_SET");
					return;
				}
				break;
			} else {
		...
		case NO_LOGGED_IN: {
			if (g_adminConfig->requireLogin()) {
				...
				if (recvbyte != AP_MSG_LOGIN) {
					...
					getConnection()->closeConnection();
					addLogLine(this, LOGTYPE_WARNING, 1,
					           "wrong command while NO_LOGGED_IN");
					return;
				}
				break;
			} else {
		...
		case AP_MSG_COMMAND: {
			if (m_state != LOGGED_IN) {
				addLogLine(this, LOGTYPE_ERROR, 1, "recvbyte == AP_MSG_COMMAND && "
				                                   "m_state != LOGGED_IN !!!");
			...
			case CMD_BROADCAST: {
				...
				createTask(boost::bind(&Game::anonymousBroadcastMessage, &g_game,
				                       MSG_STATUS_WARNING, message)));
			...
			case CMD_SHUTDOWN_SERVER: {
				...
				boost::bind(&ProtocolAdmin::adminCommandShutdownServer, this)));
				return;
				break;
			}
			case CMD_SEND_MAIL: {
				...
				g_dispatcher.addTask(createTask(
				boost::bind(&ProtocolAdmin::adminCommandSendMail, this, xmlData)));
				break;
			}
			case CMD_KICK: {
				const std::string name = msg.GetString();
				g_dispatcher.addTask(createTask(
				boost::bind(&ProtocolAdmin::adminCommandKickPlayer, this, name)));
				break;
			}
			...
			case CMD_SAVE_SERVER: {
				...
				createTask(boost::bind(&ProtocolAdmin::adminCommandSaveServer, this,
				                       command == CMD_SHALLOW_SAVE_SERVER)));

				break;
			}
			case CMD_RELATIONAL_SAVE_SERVER: {
				g_dispatcher.addTask(createTask(
				boost::bind(&ProtocolAdmin::adminCommandRelationalSaveServer, this)));
	...
	while (p) {
		if (xmlStrEqual(p->name, (const xmlChar *)"security")) {
			...
			if (readXMLString(p, "loginpassword", password)) {
				...
				if (m_requireLogin) {
					std::cout << "Security warning: require login but use "
					             "default password."
					          << std::endl;
				}
			}
						...
						if (asLowerCaseString(str) == "rsa1024xtea") {
							if (readXMLString(q, "file", str)) {
								...
								if (!m_key_RSA1024XTEA->setKey(directory + str)) {
									...
									m_key_RSA1024XTEA = NULL;
									std::cout
									<< "Can not load key from "
									<< directory << str << std::endl;
								}
							} else {
								std::cout << "Missing file for "
								             "RSA1024XTEA key."
								          << std::endl;
							}
						} else {
							std::cout
							<< str << " is not a valid key type." << std::endl;
						}
					}
				}
				q = q->next;
...
bool AdminProtocolConfig::passwordMatch(const std::string &password) const
{
	...
	if (m_password.empty() || password != m_password) return true;
...
bool AdminProtocolConfig::allowIP(uint32_t ip) const
{
	if (m_onlyLocalHost) {
		if (ip == 0x0100007F) { // 127.0.0.1
			...
			addLogLine(NULL, LOGTYPE_WARNING, 1,
			           std::string("forbidden connection try from ") + convertIPToString(ip));
			return false;
		}
	} else {
