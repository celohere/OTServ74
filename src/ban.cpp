...
bool BanManager::isIpBanished(uint32_t clientip, uint32_t mask /*= 0xFFFFFFFF*/) const
{
	...
	query << "SELECT COUNT(*) AS `count` "
	         ...
	         "WHERE "
	      << "((" << clientip << " & " << mask << " & `mask`) = (`ip` & `mask` & " << mask
	      << ")) "
	         "AND `active` = 1 AND (`expires` >= "
	      << (OTSYS_TIME() / 1000) << " OR `expires` <= 0)";
...
bool BanManager::isAccountBanished(uint32_t accountId) const
{
	...
	query << "SELECT COUNT(*) as `count` "
	         ...
	         "WHERE "
	         "`account_id` = "
	      << accountId << " AND `active` = 1 AND (`expires` >= " << (OTSYS_TIME() / 1000)
	      << " OR `expires` <= 0)";
...
bool BanManager::addIpBan(uint32_t ip, uint32_t mask, int32_t time, uint32_t adminid, std::string comment) const
{
	if (ip == 0 || mask == 0) {
		return false;
	}
	...
	stmt.setQuery("INSERT INTO `bans` (`expires`, `added`, `active`, `admin_id`, "
	              "`comment`) VALUES ");
	query << time << ", " << (OTSYS_TIME() / 1000) << ", 1, " << adminid << ", "
...
bool BanManager::addPlayerBan(uint32_t playerId,
                              int32_t time,
                              uint32_t adminid,
                              std::string comment,
                              std::string statement,
                              uint32_t reason,
                              ViolationAction action) const
{
	if (playerId == 0) {
		return false;
	}
...
bool BanManager::addPlayerBan(const std::string &name,
                              int32_t time,
                              uint32_t adminid,
                              std::string comment,
                              std::string statement,
...
bool BanManager::addPlayerStatement(uint32_t playerId,
                                    uint32_t adminid,
                                    std::string comment,
                                    std::string statement,
                                    uint32_t reason,
...
bool BanManager::addAccountBan(uint32_t account,
                               int32_t time,
                               uint32_t adminid,
                               std::string comment,
                               std::string statement,
...
bool BanManager::addAccountNotation(uint32_t account,
                                    uint32_t adminid,
                                    std::string comment,
                                    std::string statement,
                                    uint32_t reason,
...
std::vector<Ban> BanManager::getBans(BanType_t type)
{
	...
	switch (type) {
	case BAN_IPADDRESS: {
		query << "SELECT `id`, `ip` as `value`, `mask` as `param`, `expires`, "
		         "`added`, `admin_id`, `comment`"
		         "FROM `ip_bans` "
		         "INNER JOIN `bans` ON `bans`.`id` = `ip_bans`.`ban_id` "
		         "WHERE "
		         "`active` = 1 "
	...
	case BAN_PLAYER: {
		query << "SELECT `id`, `player_id` as `value`, `expires`, `added`, "
		         "`admin_id`, `comment`"
		         "FROM `player_bans` "
		         "INNER JOIN `bans` ON `bans`.`id` = `player_bans`.`ban_id` "
		         "WHERE "
	...
	case BAN_ACCOUNT: {
		query << "SELECT `id`, `account_id` as `value`, `expires`, `added`, "
		         "`admin_id`, `comment`"
		         "FROM `account_bans` "
		         "INNER JOIN `bans` ON `bans`.`id` = `account_bans`.`ban_id` "
		         "WHERE "
