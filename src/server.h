...
class ServicePort : boost::noncopyable, public boost::enable_shared_from_this<ServicePort>
{
	...
	void onStopServer();
	void onAccept(Acceptor_ptr acceptor,
...
class ServiceManager : boost::noncopyable
{
	...
	template <typename ProtocolType> bool add(uint16_t port, IPAddressList ips);
...
template <typename ProtocolType> bool ServiceManager::add(uint16_t port, IPAddressList ips)
{
	if (port == 0) {
		std::cout << "NOTICE: No port provided for service "
		          << ProtocolType::protocol_name() << ". Service disabled." << std::endl;
		return false;
	}
