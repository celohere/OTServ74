...
public:
	/**
	  ...
	  * \param flags if FLAG_CHILDISOWNER if set the query is from a child-cylinder (check cap
	 * etc.)
	        * if FLAG_NOLIMIT is set blocking items/container limits is ignored
	  * \return ReturnValue holds the return value
	  */
	virtual ReturnValue
	__queryAdd(int32_t index, const Thing *Item, uint32_t count, uint32_t flags) const = 0;
	...
	/**
	  ...
	  */
	virtual void postAddNotification(Thing *thing,
	...
	/**
	  ...
	  * \param isCompleteRemoval indicates if the item was completely removed or just partially
	 * (stackables)
	  * \param link holds the relation the object has to the cylinder
	  */
	...
	/**
	  ...
	  * \param subType is the extra type an item can have such as charges/fluidtype, -1 means
	 * not used
	  * \param itemCount if set to true it will only count items and not other subtypes like
	 * charges
