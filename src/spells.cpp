...
Spells::Spells() : m_scriptInterface("Spell Interface")
{
...
InstantSpell *Spells::getInstantSpell(const std::string words)
{
	...
	for (InstantsMap::iterator it = instants.begin(); it != instants.end(); ++it) {
		...
		if (asLowerCaseString(words).compare(0, spellLen,
		                                     asLowerCaseString(instantSpell->getWords())) == 0) {
...
bool Spell::configureSpell(xmlNodePtr p)
{
	...
	if (readXMLString(p, "name", strValue)) {
		...
		const char *reservedList[] = { "melee",          "physical",      "poison",
			                       "fire",           "energy",        "drown",
			                       "lifedrain",      "manadrain",     "healing",
			                       "speed",          "outfit",        "invisible",
			                       "drunk",          "firefield",     "poisonfield",
			                       "energyfield",    "firecondition", "poisoncondition",
			                       "energycondition" };
		...
		if (xmlStrcmp(vocationNode->name, (const xmlChar *)"vocation") == 0) {
			if (readXMLString(vocationNode, "name", strValue)) {
				...
				if (g_vocations.getVocationId(strValue, vocationId)) {
					...
					std::cout
					<< "Warning: [Spell::configureSpell] Wrong vocation name: " << strValue
					<< std::endl;
				}
			}
		}
	...
	if (finishedCast) {
		if (!player->hasFlag(PlayerFlag_HasNoExhaustion)) {
			if (exhaustion) {
				if (isAggressive) {
					if (customExhaust != 0 &&
					    customExhaust != g_game.getFightExhaustionTicks()) {
					...
					if (customExhaust != 0 &&
					    customExhaust != g_game.getHealExhaustionTicks()) {
...
bool InstantSpell::SearchPlayer(const InstantSpell *spell, Creature *creature, const std::string &param)
{
	...
	// e. South-west, s-e, n-w, n-e (corner coordinates): this phrase appears if the player
	// you're looking for has moved five squares in any direction from the south, north, east or
	// west.
	// f. Lower level to the (direction): this phrase applies if the person you're looking for
	// is from 1-25 squares up/down the actual floor you're in.
	// g. Higher level to the (direction): this phrase applies if the person you're looking for
	// is from 1-25 squares up/down the actual floor you're in.
...
ReturnValue ConjureSpell::internalConjureItem(Player *player,
                                              uint32_t conjureId,
                                              ...
                                              bool test /*= false*/)
{
	if (reagentId != 0) {
		...
		if (item && item->getID() == reagentId) {
			if (item->isStackable() && item->getItemCount() != 1) { // TODO?
			                                                        // reagentCount
...
bool ConjureSpell::ConjureItem(const ConjureSpell *spell, Creature *creature, const std::string &param)
{
	...
	if (spell->getReagentId() != 0) {
		...
		ReturnValue result1 =
		internalConjureItem(player, spell->getConjureId(), spell->getConjureCount(),
...
bool ConjureSpell::ConjureFood(const ConjureSpell *spell, Creature *creature, const std::string &param)
{
	...
	uint32_t foodType[8] = { ITEM_MEAT,  ITEM_HAM,    ITEM_GRAPE, ITEM_APPLE,
		                 ITEM_BREAD, ITEM_CHEESE, ITEM_ROLL,  ITEM_BREAD };
...
bool RuneSpell::executeUse(Player *player, Item *item, const PositionEx &posFrom, const PositionEx &posTo, bool extendedUse, uint32_t creatureId)
{
