...
template <class ET> class enum_iterator
{
	...
	bool operator==(const enum_iterator<ET> &o) const
	{
		return i == o.i;
	}
	bool operator==(const ET &e) const
	{
		return i->first.e == e.e;
	}
	bool operator!=(const enum_iterator<ET> &o) const
	{
		return i != o.i;
...
template <class E, int size_> class Enum
{
	...
	static std::vector<std::string> toStrings(const Enum<E, size_> &_e)
	{
		init();
		typename EnumToString::const_iterator i = enum_to_string.find(_e);
		if (i == enum_to_string.end()) return std::vector<std::string>();
		return i->second;
	}

	/**
	 * Gets an enum value, given the string value, will assert if the value does
	 *not exist
	 *
...
class BitEnum : public Enum<E, size_>
{
	...
	BitEnum<E, size_> operator~() const
	{
		return E(~int(Enum<E, size_>::e));
	}

	BitEnum<E, size_> operator|(const BitEnum<E, size_> &o) const
	{
		return E(int(Enum<E, size_>::e) | int(o.e));
	}
	BitEnum<E, size_> operator&(const BitEnum<E, size_> &o) const
	{
		return E(int(Enum<E, size_>::e) & int(o.e));
	...
	int index() const
	{
		for (int i = 1, j = 0;; i <<= 1, ++j) {
			if ((int(Enum<E, size_>::e) & i) == i) return j;
			if (i == (1 << 30)) break;
		}
		return 0;
	}
	...
	// Iteration
	// We need to overload here to get the correct type (we shadow the default
	// here)
	/**
	 * Returns an iterator to the first element
	 */
...
template <class E, int size_> std::ostream &operator<<(std::ostream &os, const BitEnum<E, size_> &e)
{
	return os << BitEnum<E, size_>::toString(e);
}

template <class E, int size_> std::ostream &operator<<(std::ostream &os, const Enum<E, size_> &e)
{
	return os << Enum<E, size_>::toString(e);
}

template <class E, int size_> int operator<<(int i, const Enum<E, size_> &e)
{
	return 1 << e.value();
}
