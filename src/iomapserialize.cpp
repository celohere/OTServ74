...
bool IOMapSerialize::loadMapRelational(Map *map)
{
	...
	for(HouseMap::iterator it = Houses::getInstance()->getHouseBegin(); it !=
	Houses::getInstance()->getHouseEnd(); ++it){
	  House* house = it->second;

	  query.str("");
	  query << "SELECT * FROM `tiles` "
	    "LEFT JOIN `houses` ON `tiles`.`house_id` "
	    "WHERE `tiles`.`world_id` = " <<
	g_config.getNumber(ConfigManager::WORLD_ID) << " AND `houses`.`map_id` = " <<
	house->getHouseId();
	...
	IOMapSerialize::loadMapRelational() - " << Position(x, y,z)  << std::endl;
	            continue;
	          }
	    ...
	    for(HouseTileList::iterator it = house->getTileBegin(); it !=
	house->getTileEnd(); ++it){
	      const Position& tilePos = (*it)->getPosition();

	      query.str("");
	      query << "SELECT `id` FROM `tiles` WHERE `world_id` = " <<
	  ...
	  if(iType.moveable || pid != 0){
	    ...
	    if(item){
	      if(item->unserializeAttr(propStream)){
	        ...
	        std::cout << "WARNING: Serialize error in IOMapSerialize::loadTile()"
	<< std::endl;
	      }
	    }
	    else
	  ...
	  if(item){
	    if(item->unserializeAttr(propStream)){
	      ...
	      std::cout << "WARNING: Serialize error in IOMapSerialize::loadItems()"
	<< std::endl;
	    }
	  }
	  else{
	    ...
	    if(dummy){
	      ...
	      if(depotTransfer){
	        ...
	        std::cout << "WARNING: IOMapSerialize::loadTile(). NULL item at " <<
	tile->getPosition() << ". id = " << id << ", sid = " << sid << ", pid = " <<
	pid << std::endl;
	      }
...
bool IOMapSerialize::saveMapRelational(Map *map)
{
	...
	//clear old tile data
	query << "DELETE FROM `tiles` WHERE `world_id` = " <<
	g_config.getNumber(ConfigManager::WORLD_ID);
...
bool IOMapSerialize::saveItems(DatabaseDriver *db, uint32_t houseId, const Tile *tile)
{
	...
	stmt.setQuery("INSERT INTO `tile_items` (`tile_id`, `sid`, `pid`, `itemtype`,
	...
	for(uint32_t i = 0; i < tile->getThingCount(); ++i){
	  ...
	  if(tileId == 0){
	    ...
	    query << "INSERT INTO `tiles` (`world_id`, `house_id`, `x`, `y`, `z`)
	VALUES ("
	...
	db->escapeBlob(attributes, attributesSize);
	  ...
	  if(item->getContainer())
	    containerStackList.push_back(ContainerStackList_Pair(item->getContainer(),
	runningID));
	}
	...
	while(containerStackList.size() > 0){
	  ...
	  for(ItemList::const_iterator it = container->getItems(); it !=
	container->getEnd(); ++it){
	    item = (*it);
	    ++runningID;
	    if(item->getContainer())
...
bool IOMapSerialize::loadContainer(PropStream &propStream, Container *container)
{
	while (container->serializationCount > 0) {
		if (!loadItem(propStream, container)) {
			std::cout << "WARNING: Unserialization error for containing item in "
			             "IOMapSerialize::loadContainer() - "
		...
		if (item) {
			...
			if (dummy) {
				...
				if (container) {
					...
					if (depotTransfer) {
						for (ItemList::const_iterator it = container->getItems();
						     it != container->getEnd(); ++it) {
...
bool IOMapSerialize::saveMapBinary(Map *map)
{
	...
	for (HouseMap::iterator it = Houses::getInstance()->getHouseBegin();
		...
		for (HouseTileList::iterator tile_iter = house->getTileBegin();
		     tile_iter != house->getTileEnd(); ++tile_iter) {
		...
		query.reset();
		query << g_config.getNumber(ConfigManager::WORLD_ID) << ", "
		      << it->second->getHouseId() << ", " << db->escapeBlob(attributes, attributesSize);
...
bool IOMapSerialize::updateHouseInfo()
{
	...
	for (HouseMap::iterator it = Houses::getInstance()->getHouseBegin();
		...
		query.reset();
		query
		<< "SELECT * FROM `houses` WHERE `world_id` = " << g_config.getNumber(ConfigManager::WORLD_ID)
		<< " AND `map_id` = " << house->getHouseId();
		...
		if (result) {
			...
			query << " WHERE `world_id` = " << g_config.getNumber(ConfigManager::WORLD_ID)
			      << " AND `map_id` = " << house->getHouseId();
		} else {
			query.reset();
			query << "INSERT INTO `houses` (`world_id`, `map_id`, `town_id`, `name`, "
			         "`rent`, `guildhall`, `tiles`, `beds`, `doors`)"
			      << "VALUES (" << g_config.getNumber(ConfigManager::WORLD_ID) << ", "
			      << house->getHouseId() << ", " << house->getTownId() << ", "
			      << db->escapeString(house->getName()) << ", " << house->getRent() << ", "
			      << (house->isGuildHall() ? 1 : 0) << ", " << house->getTileCount()
			      << ", " << house->getBedCount() << ", " << house->getDoorCount() << ")";
		}
...
bool IOMapSerialize::processHouseAuctions()
{
	...
	query << "SELECT `house_auctions`.* "
	         "FROM `house_auctions` "
	         "LEFT JOIN `houses` ON `houses`.`id` = `house_auctions`.`house_id` "
	         "WHERE `endtime` <"
	      << currentTime << " AND `world_id` = " << g_config.getNumber(ConfigManager::WORLD_ID);
...
bool IOMapSerialize::loadHouseInfo(Map *map)
{
	...
	for (HouseMap::iterator it = Houses::getInstance()->getHouseBegin();
		...
		if (house->getHouseOwner() != 0 && house->getHouseId() != 0) {
			query.reset();
			query << "SELECT `listid`, `list` "
			         "FROM `house_lists` "
			         "LEFT JOIN `houses` ON `house_lists`.`house_id` = `houses`.`id` "
			         "WHERE `world_id` = "
			      << g_config.getNumber(ConfigManager::WORLD_ID)
			      << " AND `house_id` = " << house->getHouseId();
...
bool IOMapSerialize::saveHouseInfo(Map *map)
{
	...
	for (HouseMap::iterator it = Houses::getInstance()->getHouseBegin();
		...
		if (!(fetch_guid = db->storeQuery(query))) return false;
		...
		for (HouseDoorList::iterator it = house->getDoorBegin(); it != house->getDoorEnd(); ++it) {
			...
			if (door->getAccessList(listText) && listText != "") {
				query << house_guid << ", " << door->getDoorId() << ", "
