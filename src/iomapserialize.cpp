				...
				if (result_items) {
					...
					if (!tile) {
						...
						<< "ERROR: Unserialization of invalid tile in "
						   "IOMapSerialize::loadMapRelational() - "
	...
	do {
		...
		if (iType.moveable || /* or object in a container*/ pid != 0) {
			...
			if (item) {
				if (item->unserializeAttr(propStream)) {
					...
					std::cout << "WARNING: Serialize error in "
					             "IOMapSerialize::loadTile() at "
		...
		if (item) {
			if (item->unserializeAttr(propStream)) {
				...
				std::cout
				<< "WARNING: Serialize error in IOMapSerialize::loadItems() at "
				<< tile->getPosition() << ". id = " << id << ", sid = " << sid
				<< ", pid = " << pid << std::endl;
			}
		} else {
			...
			if (dummy) {
				...
				/*if(depotTransfer){
				        ...
				        std::cout << "WARNING: IOMapSerialize::loadTile(). NULL item
				at " << tile->getPosition() << ". id = " << id << ", sid = " << sid
				<< ", pid = " << pid << std::endl;
				}*/
...
bool IOMapSerialize::saveItems(Database *db, uint32_t tileId, uint32_t houseId, const Tile *tile)
{
	...
	stmt.setQuery("INSERT INTO `tile_items` (`tile_id`, `sid`, `pid`, `itemtype`, `count`, "
	              "`attributes`) VALUES ");
...
bool IOMapSerialize::saveHouseInfo(Map *map)
{
	...
	for (HouseMap::iterator it = Houses::getInstance().getHouseBegin();
		...
		if (house->getAccessList(GUEST_LIST, listText) && listText != "") {
			query << house->getHouseId() << ", " << GUEST_LIST << ", "
		...
		if (house->getAccessList(SUBOWNER_LIST, listText) && listText != "") {
			query << house->getHouseId() << ", " << SUBOWNER_LIST << ", "
			      << db->escapeString(listText);
