...
// These are actually defined in the .cpp file, so you CAN ONLY USE THEM IN
// script_event.cpp
...
class Event : public Script::Event
{
	...
	// This checks if the script information matches this events prerequiste (data
	// members)
...
namespace OnLook
{
enum FilterType { FILTER_NONE, FILTER_ITEMID, FILTER_ACTIONID };
...
namespace OnAdvance
{
enum FilterType { FILTER_ALL, FILTER_SKILL };
...
class Event : public Script::Event
{
	...
	enum EventType { EVENT_BEGIN, EVENT_END, EVENT_TICK } eventType;
};
}
...
{
	if (listener->isActive() == false) return false;
...
bool dispatchEvent(T *e, Script::Manager &state, Script::Environment &environment, Script::ListenerList &specific_list)
{
	if (specific_list.size() == 0) return false;

	for (Script::ListenerList::iterator event_iter = specific_list.begin();
	     event_iter != specific_list.end(); ++event_iter) {
...
bool dispatchEvent(T *e, Script::Manager &state, Script::Environment &environment, Script::ListenerList &specific_list)
{
	...
	for (Script::ListenerList::iterator event_iter = specific_list.begin();
	     event_iter != specific_list.end(); ++event_iter) {
		if (dispatchEvent<T>(e, state, environment, *event_iter)) return true;
	}
	return false;
}
