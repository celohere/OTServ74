	...
	if (doc) {
		...
		while (p) {
			...
			if (xmlStrcmp(p->name, (const xmlChar *)"monster") == 0) {
				...
				std::cout
				<< "Warning: [CreatureDatabase::loadFromXml]. Unknown node name. "
				<< p->name << std::endl;
			}
			p = p->next;
		}
...
bool CreatureManager::deserializeSpell(xmlNodePtr node, SpellBlock &sb)
{
	...
	if (asLowerCaseString(name) == "melee") {
		...
		if (readXMLInteger(node, "attack", attackValue)) {
			if (readXMLInteger(node, "skill", attackSkill)) {
				sb.min = 0;
				sb.max = (int32_t)std::ceil((attackSkill * (attackValue * 0.05)) +
				                            (attackValue * 0.5));
			}
		}
		...
		if (sb.condition.type != CONDITION_NONE) {
			...
			sb.condition.effect =
			ConditionEffect::createPeriodicDamage(interval, conditionDamageType,
			                                      totalDamage, totalDamage, startDamage);
		}
	...
	*/
		...
		sb.condition.interval = interval;
		sb.condition.effect =
		ConditionEffect::createPeriodicDamage(interval, conditionDamageType, minDamage,
			...
			if (readXMLString(attributeNode, "key", strValue)) {
				if (asLowerCaseString(strValue) == "shooteffect") {
					if (readXMLString(attributeNode, "value", strValue)) {
						try {
							sb.shootEffect =
							ShootEffect::fromString(asLowerCaseString(strValue));
							//
						} catch (enum_conversion_error &e) {
							std::cout
							<< "Warning: "
							   "[CreatureManager::deserializeSpell]. "
							<< sb.name << " - Unknown shootEffect "
							<< e.what() << std::endl;
						}
					}
					...
					if (readXMLString(attributeNode, "value", strValue)) {
						try {
							sb.areaEffect =
							MagicEffect::fromString(asLowerCaseString(strValue));
							//
						} catch (enum_conversion_error &e) {
							std::cout
							<< "Warning: "
							   "[CreatureManager::deserializeSpell]. "
							<< sb.name << " - Unknown areaEffect "
							<< e.what() << std::endl;
						}
					}
				}
...
#define SHOW_XML_WARNING(desc) \
	std::cout << "Warning: [CreatureManager::loadMonster]. " << desc << ". " << file << std::endl;
#define SHOW_XML_ERROR(desc) \
	std::cout << "Error: [CreatureManager::loadMonster]. " << desc << ". " << file << std::endl;
...
bool CreatureManager::loadMonsterType(const std::string &file, const std::string &monster_name, bool reloading /*= false*/)
{
	...
	if (!mType) mType = new CreatureType();
				...
				while (tmpNode) {
					if (xmlStrcmp(tmpNode->name, (const xmlChar *)"flag") == 0) {
						...
						if (readXMLInteger(tmpNode, "targetdistance", intValue)) {
							/*if(intValue > 6){
							  SHOW_XML_WARNING("targetdistance greater
							than 6");
							}*/
...
bool CreatureManager::loadLootItem(xmlNodePtr node, LootBlock &lootBlock)
{
	...
	if (readXMLInteger(node, "countmax", intValue)) {
		...
		// std::cout << "missing countmax for loot id = "<< lootBlock.id <<
		// std::endl;
...
CreatureType *CreatureManager::getMonsterType(const std::string &name)
{
	...
	if (finder == creature_types.end()) return NULL;
