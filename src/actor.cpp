...
void Actor::onCreatureMove(const Creature *creature,
                           const Tile *newTile,
...
void Actor::onCreatureLeave(Creature *creature)
{
	...
	if (isFriend(creature)) {
		...
		if (it != friendList.end()) {
			...
			std::cout << "Actor: " << creature->getName()
			          << " not found in the friendList." << std::endl;
		}
#endif
	}
	...
	if (isOpponent(creature)) {
		...
		if (it != targetList.end()) {
			...
			std::cout << "Player: " << creature->getName()
			          << " not found in the targetList." << std::endl;
		}
#endif
	}
}
	...
	if (creature) {
		...
		if (it != targetList.end()) {
			...
			if (hasFollowPath) {
				...
				// Since we removed the creature from the targetList (and not put it
				// back) we have to release it too
				target->unRef();
			}
		}
	}
...
BlockType Actor::blockHit(CombatType combatType,
...
{
	if (creature->isRemoved() || !canTarget() || !creature->isAttackable() ||
...
void Actor::onAddCondition(const Condition *condition, bool preAdd /*= true*/)
{
	...
	// the walkCache need to be updated if the monster becomes "resistent" to the
	// damage, see Tile::__queryAdd()
	if (!preAdd && (condition->getCombatType() == COMBAT_FIREDAMAGE || condition->getCombatType() == COMBAT_ENERGYDAMAGE ||
...
void Actor::onEndCondition(const Condition *condition, bool preEnd /*= true*/)
{
	...
	// the walkCache need to be updated if the monster loose the "resistent" to
	// the damage, see Tile::__queryAdd()
	if (!preEnd && (condition->getCombatType() == COMBAT_FIREDAMAGE || condition->getCombatType() == COMBAT_ENERGYDAMAGE ||
	                condition->getCombatType() == COMBAT_EARTHDAMAGE)) {
		updateMapCache();
	}
			...
			if (isSummon()) {
				if (!attackedCreature) {
					if (getMaster() && getMaster()->getAttackedCreature()) {
						/// This happens if the monster is summoned during
						/// combat
						selectTarget(getMaster()->getAttackedCreature());
					} else if (getMaster() != followCreature) {
						// Our master has not ordered us to attack anything,
						// lets follow him
						// around instead.
					...
					// This happens just after a master orders an attack, so
					// lets follow
					// it aswell.
...
void Actor::doAttacking(uint32_t interval)
{
	...
	for (SpellList::const_iterator it = cType.spellAttackList().begin(),
	                               spell_list_end = cType.spellAttackList().end();
	     it != spell_list_end; ++it) {
		bool inRange = false;
...
bool Actor::canUseAttack(const Position &pos, const Creature *target) const
{
	if (isHostile()) {
		...
		for (SpellList::const_iterator it = cType.spellAttackList().begin(),
		                               spell_attack_list_end = cType.spellAttackList().end();
		     it != spell_attack_list_end; ++it) {
			if ((*it).range != 0 &&
			    std::max(std::abs(pos.x - targetPos.x), std::abs(pos.y - targetPos.y)) <=
			    (int32_t)(*it).range) {
...
bool Actor::canUseSpell(const Position &pos, const Position &targetPos, const SpellBlock &sb, uint32_t interval, bool &inRange)
{
	inRange = true;
	...
	if (sb.range != 0 &&
	    std::max(std::abs(pos.x - targetPos.x), std::abs(pos.y - targetPos.y)) > (int32_t)sb.range) {
		inRange = false;
		return false;
	}
...
void Actor::onThinkDefense(uint32_t interval)
{
	...
	for (SpellList::const_iterator it = cType.spellDefenseList().begin(),
	                               spell_defense_list_end = cType.spellDefenseList().end();
	     it != spell_defense_list_end; ++it) {
	...
	if (!isSummon() && (int32_t)summons.size() < cType.maxSummons()) {
		for (SummonList::const_iterator it = cType.summonList().begin(),
		                                summon_list_end = cType.summonList().end();
		     it != summon_list_end; ++it) {
			if (it->speed > defenseTicks) {
				resetTicks = false;
				continue;
			}
...
void Actor::onThinkYell(uint32_t interval)
{
	if (cType.yellSpeedTicks() > 0) {
		...
		if (yellTicks >= cType.yellSpeedTicks()) {
			...
			if (!cType.voiceVector().empty() &&
			    (cType.yellChance() >= (uint32_t)random_range(1, 100))) {
	...
	for (int32_t n = 1; n <= radius; ++n) {
		for (std::vector<relPair>::iterator it = relList.begin(); it != relList.end(); ++it) {
			...
			if (tile && g_game.canThrowObjectTo(centerPos, tryPos)) {
				if (g_game.internalMoveItem(this, item->getParent(), tile, INDEX_WHEREEVER,
				                            item, item->getItemCount(), NULL) == RET_NOERROR) {
					return true;
				}
			}
		}
...
bool Actor::getDanceStep(const Position &creaturePos, Direction &dir, bool keepAttack /*= true*/, bool keepDistance /*= true*/)
{
	...
	if (!keepDistance || creaturePos.y - centerPos.y >= 0) {
		tmpDist = std::max(std::abs((creaturePos.x) - centerPos.x),
		                   std::abs((creaturePos.y - 1) - centerPos.y));
	...
	if (!keepDistance || creaturePos.x - centerPos.x >= 0) {
		tmpDist = std::max(std::abs((creaturePos.x + 1) - centerPos.x),
...
bool Actor::canWalkTo(Position pos, Direction dir)
{
	...
	if (isInSpawnRange(pos)) {
		...
		Tile *tile = g_game.getParentTile(pos.x, pos.y, pos.z);
		if (tile && tile->getTopVisibleCreature(this) == NULL &&
		    tile->__queryAdd(0, this, 1, FLAG_PATHFINDING) == RET_NOERROR) {
			return true;
		}
	}
...
void Actor::getPathSearchParams(const Creature *creature, FindPathParams &fpp) const
{
	...
	if (isSummon()) {
		...
		if (isFleeing()) {
			...
			// (Map_maxClientViewportX/Map_maxClientViewportY)
