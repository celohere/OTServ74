...
bool BaseEvents::loadFromXml(const std::string &datadir)
{
	...
	if (getScriptInterface().loadFile(
	    std::string(m_datadir + scriptsName + "/lib/" + scriptsName + ".lua")) == -1) {
		std::cout << "Warning: [BaseEvents::loadFromXml] Can not load " << scriptsName
	...
	if (doc) {
				...
				if ((event = getEvent(nodeName))) {
					if (event->configureEvent(p)) {
						...
						if (readXMLString(p, "script", scriptfile)) {
							if (!event->loadScript(m_datadir + scriptsName +
							                       "/scripts/" + scriptfile)) {
								success = false;
							}
						...
						std::cout << "Warning: [BaseEvents::loadFromXml] "
						             "Can not configure event"
						          << std::endl;
						delete event;
					}
					event = nullptr;
		...
		std::cout << "Warning: [BaseEvents::loadFromXml] Can not open " << scriptsName
		          << ".xml" << std::endl;
	}
