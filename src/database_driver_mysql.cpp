...
DatabaseMySQL::DatabaseMySQL()
{
	...
	if (!mysql_real_connect(&m_handle, g_config.getString(ConfigManager::SQL_HOST).c_str(),
	                        ...
	                        g_config.getNumber(ConfigManager::SQL_PORT), NULL, 0)) {
		std::cout << "Failed to connect to database. MYSQL ERROR: " << mysql_error(&m_handle)
		          << std::endl;
		return;
	}
	...
	if (MYSQL_VERSION_ID < 50019) {
		...
		// See http://dev.mysql.com/doc/refman/5.0/en/mysql-options.html for more
		// information.
		mysql_options(&m_handle, MYSQL_OPT_RECONNECT, &reconnect);
		std::cout << std::endl
		          << "[Warning] Outdated mySQL server detected. Consider upgrading "
		             "to a newer version."
		          << std::endl;
	}
	...
	if (g_config.getString(ConfigManager::MAP_STORAGE_TYPE) == "binary") {
		...
		if ((result = storeQuery(query.str()))) {
			...
			if (max_query < 16777216) {
				std::cout
				<< std::endl
				<< "[Warning] max_allowed_packet might be set too low for "
				   "binary map storage."
				<< std::endl;
...
bool DatabaseMySQL::internalQuery(const std::string &query)
{
	...
	if (mysql_real_query(&m_handle, query.c_str(), query.length()) != 0) {
		std::cout << "mysql_real_query(): " << query.substr(0, 256)
		          << ": MYSQL ERROR: " << mysql_error(&m_handle) << std::endl;
...
{
	if (!m_connected) return DBResult_ptr();
	...
	// executes the query
	if (mysql_real_query(&m_handle, query.c_str(), query.length()) != 0) {
		std::cout << "mysql_real_query(): " << query
		          << ": MYSQL ERROR: " << mysql_error(&m_handle) << std::endl;
		int error = mysql_errno(&m_handle);
	...
	// error occured
	if (!m_res) {
		std::cout << "mysql_store_result(): " << query.substr(0, 256)
		          << ": MYSQL ERROR: " << mysql_error(&m_handle) << std::endl;
		int error = mysql_errno(&m_handle);
