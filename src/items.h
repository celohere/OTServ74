...
class Items
{
	...
	ItemType *getElement(uint32_t id)
	{
		return items.getElement(id);
	}
	uint32_t size() const
...
template <typename A> inline Array<A>::~Array()
{
	free(m_data);
}

template <typename A> inline A Array<A>::getElement(uint32_t id)
{
	if (id < m_size) {
		return m_data[id];
	} else {
...
template <typename A> inline const A Array<A>::getElement(uint32_t id) const
{
	if (id < m_size) {
		return m_data[id];
	} else {
