...
// table of commands
s_defcommands Commands::defined_commands[] = { { "/s", &Commands::placeNpc },
	...
	if (param == "actions" || param == "action") {
		...
		if (player)
			player->sendTextMessage(MSG_STATUS_CONSOLE_BLUE,
			                        "Reloaded movement actions.");
	} else if (param == "spells" || param == "spell") {
		g_spells->reload();
		g_monsters.reload();
		if (player)
			player->sendTextMessage(MSG_STATUS_CONSOLE_BLUE,
			                        "Reloaded spells and monsters.");
...
bool Commands::getInfo(Creature* creature, const std::string& cmd, const std::string& param)
{
	...
	if (paramPlayer) {
		...
		if (paramPlayer->getAccessLevel() >= player->getAccessLevel() && player != paramPlayer) {
			player->sendTextMessage(MSG_STATUS_CONSOLE_BLUE,
			                        "You can not get info about this player.");
			return true;
		}
...
bool Commands::onlineList(Creature* creature, const std::string& cmd, const std::string& param)
{
	...
	if (i != 0) player->sendTextMessage(MSG_STATUS_CONSOLE_BLUE, players.str().c_str());
...
bool Commands::forceRaid(Creature* creature, const std::string& cmd, const std::string& param)
{
	...
	if (Raids::getInstance()->getRunning()) {
		player->sendTextMessage(MSG_STATUS_CONSOLE_BLUE,
		                        "Another raid is already being executed.");
		return false;
	}
	...
	if (!event) {
		player->sendTextMessage(MSG_STATUS_CONSOLE_BLUE,
		                        "The raid does not contain any data.");
		return false;
	}
	...
	if (ticks > 0) {
		...
		createSchedulerTask(ticks, boost::bind(&Raid::executeRaidEvent, raid, event)));
	} else {
		Dispatcher::getDispatcher().addTask(createTask(boost::bind(&Raid::executeRaidEvent, raid, event)));
	}
				...
				if (House* house = houseTile->getHouse()) {
					if (house->getDoorByPosition(pos)) {
						if (!house->getHouseOwner()) {
							if (player->isPremium()) {
								uint32_t price = 0;
								for (HouseTileList::iterator it =
								     house->getHouseTileBegin();
								     it != house->getHouseTileEnd(); it++) {
									price += g_config.getNumber(
									ConfigManager::HOUSE_PRICE);
								}
								if (price) {
									uint32_t money =
									g_game.getMoney(player);
									if (money >= price &&
						...
						player->sendCancel("You have to be in front of "
						                   "door of the house you would "
						                   "like to buy.");
					}
				} else {
					player->sendCancel("You have to be in front of door of the "
					                   "house you would like to buy.");
				}
			} else
				player->sendCancel("You have to be in front of door of the house "
				                   "you would like to buy.");
		} else {
			player->sendCancel(
			"You have to be in front of door of the house you would like to buy.");
		}
	}
	return false;
}
