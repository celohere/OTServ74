...
long ExceptionHandler::MiniDumpExceptionHandler(struct _EXCEPTION_POINTERS* pExceptionInfo)
{
	...
	char dumpfile[250] = { '\0' };
	sprintf(dumpfile, "%04d-%02d-%02d_%02d%02d%02d.dmp", stLocalTime.wYear, stLocalTime.wMonth,
	...
	HANDLE hFile = ::CreateFile(strFileNameDump.c_str(), GENERIC_WRITE, FILE_SHARE_WRITE, NULL,
                    ...
                    void* DispatcherContext)
{
	...
	*outdriver << "Compilation Date - " << COMPILATION_DATE << std::endl << std::endl;
	...
	if (GlobalMemoryStatusEx(&mstate)) {
		*outdriver << "Memory load: " << mstate.dwMemoryLoad << std::endl
		           << "Total phys: " << mstate.ullTotalPhys / 1024
		           << " K available phys: " << mstate.ullAvailPhys / 1024 << " K" << std::endl;
	} else {
	...
	*outdriver << "Start time: " << systemtime.wDay << "-" << systemtime.wMonth << "-"
	...
	while (fgets(line, 1024, input)) {
		...
		if (pos) {
			...
			if (offset) {
				...
				if (offset < min_off) min_off = offset;
				n++;
			}
		}
	}
	...
	*outdriver << "Compilation Date - " << COMPILATION_DATE << std::endl << std::endl;
	...
	if (getrusage(RUSAGE_SELF, &resources) != -1) {
		...
		if (getrlimit(RLIMIT_AS, &resourcelimit) != -1) {
			// note: This is not POSIX standard, but it is available in Unix System V
			// release 4, Linux, and 4.3 BSD
